...
public class JobServiceImpl implements JobService{
	...
	public HashMap<String, Object>  searchJobByKeyWord(JobListInput jobInput) {
	   ...
	   /* StringBuffer changkeyword = new StringBuffer(jobInput.getKeyword());
	    StringBuffer finalkeyword = new StringBuffer(jobInput.getKeyword());
		...
		String  str ="_";
		    ...
		    for(int i=0;i<strList.size();i++) {
		    	str=strList.get(i);
		    	 changkeyword.replace(0, changkeyword.length(), oldkeyword);
		    	while (changkeyword.indexOf(str)!=-1){
				        //截取包含自身在内的前边部分
				        String aa= changkeyword.substring(0,changkeyword.indexOf(str)+str.length());
				        flag=flag+aa.length();
				        list.add(flag-str.length());
				        oldkeyword=changkeyword.substring(changkeyword.indexOf(str)+str.length());
				}
		    }
		   
		    StringBuffer finalkeyword = new StringBuffer(jobInput.getKeyword());
		    for(int i =list.size()-1;i>=0;i--) {
		    	finalkeyword.insert(list.get(i), "!");
		    }*/
