	 ...
	 * @param model 模型对象
	 * @param req request对象
	 <#if paging || (inParamList?? && inParamList?size != 0)  >
	 * @param reqVo 请求参数
	 </#if>
	 */
	...
	public void ${path}(ModelMap model, HttpServletRequest req
			...
			,@RequestBody ${controllerInputBeanName} reqVo
			<#else>
				<#if paging>
			,@RequestBody BaseVo reqVo
				</#if>
			</#if>) {
		<#if inParamList?? && inParamList?size != 0>
		LogCvt.info("${name}请求参数：" + JSON.toJSONString(reqVo));
		<#else>
		LogCvt.info("${name}请求参数：{}");
		</#if>
		...
		try {
			...
			// 调用服务
			model.putAll(${controllerServiceName?uncap_first}.${path}(clientId<#if paging || (inParamList?? && inParamList?size != 0)  >, reqVo</#if>));
		} catch (TException e) {
