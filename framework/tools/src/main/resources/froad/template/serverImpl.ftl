...
public class ${serverName}ServiceImpl extends BizMonitorBaseService implements ${serverName}Service.Iface {
	...
	public ResultVo ${path}(${serverInputBeanName} ${serverInputBeanName?uncap_first}) throws TException {
		...
		LogCvt.debug("${name}接口输入参数,${serverInputBeanName?uncap_first}:" + JSON.toJSONString(${serverInputBeanName?uncap_first}));
		ResultVo result = new ResultVo();
		try {
	<#else>
	public ${serverOutputBeanName} ${path}(${serverInputBeanName} ${serverInputBeanName?uncap_first}, PageVo pageVo) throws TException {
		long startTime = System.currentTimeMillis();
		LogCvt.debug("${name}接口输入参数,${serverInputBeanName?uncap_first}:" + JSON.toJSONString(${serverInputBeanName?uncap_first}) + ",pageVo:" + JSON.toJSONString(pageVo));
		ResultVo result = new ResultVo();
