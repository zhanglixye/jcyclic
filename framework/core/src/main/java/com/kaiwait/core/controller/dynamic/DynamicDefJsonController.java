...
import com.kaiwait.utils.common.ConfigParameterUtil;
...
public class DynamicDefJsonController {
	...
	//private String serverIP = "192.168.0.141";
	//逻辑服务器后台负载均衡器地址 localhost   local test server
	private String serverIPlocal = "localhost";
	
	//Redis服务器IP地址jcyclic.cn  HC  aliyun
	//内网：r-2ze0u3s44ce4cp71vf.redis.rds.aliyuncs.com
	//外网：r-2ze0u3s44ce4cp71vfpd.redis.rds.aliyuncs.com
    //private static String ADDR = "r-2ze0u3s44ce4cp71vf.redis.rds.aliyuncs.com";
	//private static String jedisPwd = "qwe123A!2020";
    
    //Redis服务器IP地址j-cyclic.com.cn  HW  aliyun
	//内网：r-2zee0e2emiw2csqtd9.redis.rds.aliyuncs.com
	//外网：r-2zee0e2emiw2csqtd9pd.redis.rds.aliyuncs.com
	//private static String ADDR = "r-2zee0e2emiw2csqtd9.redis.rds.aliyuncs.com";
	//private static String jedisPwd = "qwe123A!2020";
	...
	public ResponseEntity<String> dispatcher(@PathVariable String action, HttpServletRequest request) {
		...
		ConfigParameterUtil.init(this.getClass());
...
//		}
	...
	private ResponseEntity<String> dynamicInvokeThriftJsonService(String action, HttpServletRequest request) {
		...
		try {
			...
			if(action.equals("logout") && session != null)
			{
				...
				//JsonNode rt = OBJECT_MAPPER.readTree(sb.toString());
				//String value = rt.get(MessageWarp.DATA_FIELD).path("userName").textValue();
				//Double value1 = Double.valueOf(value);
				//DecimalFormat df = (DecimalFormat)NumberFormat.getInstance();
				//df.applyPattern("####.00");
				//df.format(value1);
				//[a-zA-Z0-9\u4E00-\u9FA5\uFF61-\uFF9F\uFFE8-\uFFEE\u3040-\u309F\uFF10-\uFF19\uFF21-\uFF3A\uFF41-\uFF5A\u30A0-\u30FF]*
				//String regEx = "[a-zA-Z0-9\\u4E00-\\u9FA5\\uFF61-\\uFF9F\\uFFE8-\\uFFEE\\u3040-\\u309F\\uFF10-\\uFF19\\uFF21-\\uFF3A\\uFF41-\\uFF5A\\u30A0-\\u30FF]*";
				//String regEx="[`~!@#$%^&*()+=|{}':;',\\[\\].<>/?~！@#￥%……&*（）——+|{}【】‘；：”“’。，、？]*"; 
				...
				{
					System.out.println("serverIPlocal------------"+serverIPlocal);
					switch (requestMeta.getCustId()) {
						case MessageWarp.REQUEST_MST_NAME:
							...
							//responseJson = DynamicJsonServiceClient.invoke(serverIPlocal, 9091, 30000, sb.toString());
							break;
						case MessageWarp.REQUEST_JCZH_NAME:
							responseJson = DynamicJsonServiceClient.invoke(serverIPlocal, 8082, 120000, sb.toString());
							//responseJson = DynamicJsonServiceClient.invoke(serverIPlocal, 9092, 30000, sb.toString());
							break;
						default :
		...
		try {
			...
			case OK:
				if(action.equals("systemlock") && session != null)
				{
				...
				{
					lockStatus = (String)session.getAttribute("systemlock");
				}
				webOutputMeta = new WebOutputMeta();
				webOutputMeta.setResult(Result.OK);
	...
	private ResponseEntity<String> dynamicInvokeThriftJsonService(String action, HttpServletRequest request) {
		...
		try {
			...
			if(!action.equals("changeCompany"))
			{
			...
			if(action.equals("logout") && session != null)
			{
				...
				if(action.equals("changeCompany") && session != null)
				{
					companySysLock = "systemlock"+rt.get(MessageWarp.DATA_FIELD).path("changeCompanyID").textValue();
				}
		...
		try {
			...
			case OK:
				...
				if(lockStatus.equals("") && jedis != null)
				{
