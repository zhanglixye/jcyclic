function initShow() {
	...
	if(nowUrl.match(/\/salescategory_update.html/) != null) {
		jobcd = getQueryString("jobcd", "jczh/salescategory_update.html");
	}
	if(nowUrl.match(/\/salescategory_registration.html/) != null) {
		jobcd = getQueryString("jobcd", "jczh/salescategory_registration.html");
	}
		...
		success: function(data) {
			if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
				...
				if(data[$.getRequestDataName()] != null) {
					...
					saleStorage.recLockFlg = data[$.getRequestDataName()].recLockFlg;
					var job = data[$.getRequestDataName()]["jobLandVo"].jobland;
					saleStorage.firstloginms = "1";
					
					saleStorage.plan_vat_amt = job['plan_vat_amt']
					saleStorage.plan_sale = job['plan_sale']
					saleStorage.plan_sale_foreign_amt =job['plan_sale_foreign_amt']
					...
					saleStorage.newVatRate = data[$.getRequestDataName()]["jobLandVo"]['jobland'].vat_rate;
					//页面加载设置job信息
					...
					if(data[$.getRequestDataName()].job_cd!=null){
						saleStorage.enddata = job['plan_dalday'];
						var sale_id = job['sale_typ'];
						var arrSaleRate = getSaleRate(sale_id);
						saleStorage.oldVatRate = arrSaleRate['vat_rate'];
						var ifSaleRate = part_language_change_new('IF_SALE_RATE')
						if(saleStorage.oldVatRate != saleStorage.newVatRate){
							$.messager.confirm('Confirmation', ifSaleRate, function(r) {
									if(r) {
										saleStorage.firstlogin="1";
										saleStorage.firstloginms = "0";
										saleStorage.sale_vat_rate = saleStorage.oldVatRate;	
										//设置売上实际信息
										...
										saleStorage.sale_vat_rate1 = data[$.getRequestDataName()]["jobLandVo"]['jobland'].sale_vat_rate;
										
									}else{
										saleStorage.firstlogin="1";
										saleStorage.sale_vat_rate = saleStorage.newVatRate;
										//设置売上实际信息
										if(saleno != null && saleno != '' && saleno != undefined) {
											setRealSaleMessage(data[$.getRequestDataName()]);
											//变换卖上更新卡片的 name属性
...
//						$('.tax_total').text(formatNumber(data['vat_amt']));
						...
						saleStorage.sale_vat_rate1 = data[$.getRequestDataName()]["jobLandVo"]['jobland'].sale_vat_rate;
						saleStorage.sale_vat_rateold = data[$.getRequestDataName()]["jobLandVo"]['jobland'].sale_vat_rate;
...
//控制计上日，营业额控件。若请求指示书发行請求指示書・発票発行依頼書,的其中一个发行了，
//营业是不能修改「計上日」「売上額」的
//管理・財務可以修改  
 
function timeboxcontrol(data){
	if(data[$.getRequestDataName()]['req']!=null||data[$.getRequestDataName()]['inv']!=null){
		if(data[$.getRequestUserInfoName()]['departCD']!="001"&&data[$.getRequestUserInfoName()]['departCD']!="002"){
			$('#dalday').datebox('disable', true);//输入框
			$('#pcost').numberbox('disable', true);//计上日
			$('#sale_del').attr("disabled",true);
			...
			$(".sale_rate_click").unbind('click');
			$(".sale_rate_click").find('label.active').removeClass('active').addClass('activeT');
			$('.panel-body.panel.switch').css('background-color','#fff');
			$('.panel-body.panel.switch').next().css('background','inherit');
			$('.bus_color').css('background-color','white');
			$(".plan_sale_cure_code").attr("disabled",true);//換算レート
			$('#common-day').datebox('disable', true);//適用日
			$(".plan_sale_refer").attr("disabled",true);//参照先
			$("[name='jobregistration_edit']").remove();//编辑按钮
		}
	}
}
//权限控制 发行书部分
...
function getSaleRate(sale_id) {
	...
	if(date1 == '' || date1 == null) {
		if(saleStorage.enddata!=null){
			date1 = saleStorage.enddata
		}
		else{
			showErrorHandler("DATE_NOT_FILL", "info", "info");
			return false;
		}
...
//			console.log(data);
			if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
				...
				if(data[$.getRequestDataName()] != null) {
					...
					if(saleMessage.rate3 == null) {
						if(!$('.window-mask').exist()){
							showErrorHandler("NOTFINDE_TAX", "warning", "warning");
						}	
						// $("#dalday").datebox('setValue','');
						return;
					}
	...
	if(saleVatRate['vat_rate'] == undefined || saleVatRate['vat_rate'] == null || saleVatRate['vat_rate'] == "" ||
		...
		saleStorage.oldVatRate = saleVatRate;
	...
	for(var i = 0; i < card_list.length; i++) {
     ...
     var colorStyle = "color:"+card_list[i]['usercolor'];
		...
		if(card_list[i]['level_flg'] == 1 || card_list[i]['level_flg'] == 2) {
			str += '<span class="text-center block i18n" name="jobregistration_business_ren" >'+part_language_change_new('jobregistration_business_ren')+'</span>';
		} else {
			str += '<span class="text-center block i18n" name="jobregistration_bear_ren" >'+part_language_change_new('jobregistration_bear_ren')+'</span>';
		}
...
function initCal(){
    var saleMoney = recoveryNumber($('#sale_foreign_amt').text());//卖上金额
	...
	saleStorage.sale_vat_rate2 =  arrSaleRate['vat_rate'];
...
function setRealSaleMessage(data) {
	...
	if(saleStorage.sale_vat_rate==saleStorage.oldVatRate.vat_rate){
		if(saleStorage.firstloginms=="1"){
			$('#sale_foreign_amt').text(formatNumber(data['sale_foreign_amt']));	
		}
		 
		
	}else{
		if(saleStorage.firstlogin=="1"){
			if(data['sale_foreign_amt']!=$('#sale_foreign_amt').text()){
				 $('#sale_foreign_amt').text(formatNumber(data['sale_foreign_amt']));
			}
		}
		...
		if(saleStorage.sale_vat_rate==saleStorage.oldVatRate.vat_rate){
			...
			if(saleStorage.firstlogin=="1"){
				if(data['sale_vat_amt']!=$('#sale_tax').text()){
					$('#sale_tax').text(formatNumber(data['sale_vat_amt']));
					//请款金额
					$('#req_amt').text(formatNumber(data['req_amt']));
					//税金
...
function getSaleMessage() {
	...
	if(saleStorage.sale_vat_rate!=null){
		sale['vat_rate'] = saleStorage.sale_vat_rate;
	}
	else{
		sale['vat_rate'] = saleStorage.sale_vat_rate2
	}
	//判断是否卖上 承认跳过
...
//	console.log(inputVerfy());
	...
	if(!flg) {
		//showErrorHandler("REQUIRED_ITEM_NOT_FILLED", "info", "info");
		return;
	}
	...
	saleMsg['recLockFlg'] = saleStorage.getRecLockFlg();
	var sale = {
		saleType: saleMsg
	}
			...
			if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
				...
				if(data[$.getRequestDataName()] > 0) {
					...
					if(data[$.getRequestDataName()] == 999) {
						showErrorHandler('DATE_RANGE_ERROR', 'warning', 'warning');
						return;
					}
					var url = "jczh/job_registration_list.html?view=init&menu=se";
					...
					switch(bol) {
						case 0:
							...
							break;
						case 1:
							OutPutPdfHandler(jobcd, "", "", "invoiceApplication", "","","",0);
							break;
						case 2:
							OutPutPdfHandler(jobcd, "", "", "billOrder", "", "","",0);
							break;
						case 3:
							str = "invoiceApplication";
							OutPutPdfHandler(jobcd, "", "", "billOrder", "", str,"",0);
							break;
...
function calculationSale(saleIsHave){
  ...
  if(saleActive==1){
	...
	//卖上增值税率
	 ...
	 if(saleStorage.firstlogin=="1"){
		 if(saleStorage.sale_vat_rate!=null){
			 var saleVatRate =  saleStorage.sale_vat_rate
		 }
		 else{
			 var saleVatRate = arrSaleRate['vat_rate'];
		 }
	...
	if(saleStorage.newsale != "1"){
	$('.plan_sale_foreign_amt').text(sale_obj['saleBase'] == null ? Number(0).toFixed(point) : formatNumber(sale_obj['saleBase']));
	$('.plansale_tax').text(sale_obj['saleVatBase'] == null ? Number(0).toFixed(point): formatNumber(sale_obj['saleVatBase']));
	$('.plan_req_amt').text(sale_obj['reqAmtBase'] == null ? Number(0).toFixed(point) : formatNumber(sale_obj['reqAmtBase']));
	}
	if(saleStorage.isRecalculate==1){
		$('.plansale_tax').text(saleStorage.sale_tax);
		$('.plan_sale_foreign_amt').text(saleStorage.sale_foreign_amt); 
	    $('.plan_req_amt').text(saleStorage.req_amt);
	}
	//实际卖上增值税
	saleStorage.realSaleTax = formatNumber(sale_obj['saleVatBase']);
	saleStorage.realSaleTaxFlg = 0;
...
function bindAction() {
	...
	$('#dalday').datebox({
		onChange: function(date) {
			var plan_dlvday = $('#dalday').datebox('getValue');
...
function delSale() {
	...
	var jobcd = $(".job_cd").text();
	var saleLockFlg = saleStorage.getSaleLockFlg();
	var recLokcFlg = saleStorage.getRecLockFlg();
	var cd = {
		job_cd: jobcd,
		saleLockFlg:saleLockFlg,
		recLokcFlg:recLokcFlg
	}
	$.ajax({
		url: $.getAjaxPath() + "delSale",
...
function upRateArea() {
	...
	//如果在原价预计时去改变增值税  应该用预计的去算  强制再点击一遍 原价预计 
	if($(".model_fourth").hasClass("active")){
		 $(".model_fourth").click();
	}
...
function planMoveSet(){
	 var ifSaleRate = part_language_change_new('IF_SALE_RATE')
	if(saleStorage.sale_vat_rate1 != saleStorage.sale_vat_rate2){
		$.messager.confirm('Confirmation', ifSaleRate, function(r) {
			if(r) {
				saleStorage.firstlogin="1";
				saleStorage.sale_vat_rate = saleStorage.sale_vat_rate2
				//所在页面
				var saleActive = $('.sale_head').eq(0).find(".active").find("input").val();
			    //	外货code
			   ...
			   saleStorage.firstlogin="0";
				...
				saleStorage.firstlogin="1";
				saleStorage.sale_vat_rate =saleStorage.sale_vat_rateold;
				//所在页面
				var saleActive = $('.sale_head').eq(0).find(".active").find("input").val();
			    //	外货code
			    var moneyCode =  $('#plansale_foreign_code_off').val();
				...
				if($('.plan_sale_foreign_amt').text()!=saleStorage.plan_sale){
					$('.plan_sale_foreign_amt').text(formatNumber(saleStorage.plan_sale))
					saleStorage.newsale = "1";
				}
				if($('.plansale_tax').text()!=saleStorage.plan_vat_amt){
					$('.plansale_tax').text(formatNumber(saleStorage.plan_vat_amt))
					saleStorage.newsale = "1";
				}
				var saleReqAmt = floatObj.add(parseFloat(saleStorage.plan_sale),parseFloat(saleStorage.plan_vat_amt));
				if($('.plan_req_amt').text()!=saleReqAmt){
					$('.plan_req_amt').text(formatNumber(saleReqAmt))
					saleStorage.newsale = "1";
				}
			   $(".model_first").click();
			   saleStorage.newsale ="";
		...
		saleStorage.sale_vat_sale = saleStorage.sale_vat_rateold;
		//所在页面
		var saleActive = $('.sale_head').eq(0).find(".active").find("input").val();
	    //	外货code
	    var moneyCode =  $('#plansale_foreign_code_off').val();
...
$(function() {
	$('#language').change(function() {
		...
		var itemname ="";
			...
			$('.person-m-m .userlevel').eq(i).text(itemname);
...
saleStorage = {
	...
	getRecLockFlg:function(){
		return this.recLockFlg;
	}
}