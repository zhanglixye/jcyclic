 ...
 function getSaleRate(sale_id) {
 	...
 	if(date1 == '' || date1 == null) {
 		...
 		if(objStorage.enddata!=null){
			date1 =objStorage.enddata;
		}
 		else{
 			zeroSet();
 	 		return false;
...
// 			console.log(data);
 ...
 function calculationSale(saleIsHave) {
 	...
 	if($('#switch-state').bootstrapSwitch('state')){	
 		if(saleCurCode==""){
 	 		saleCurCode = objStorage.hsnumbersale;
 	 	}
 	}
 	//卖上增值税率
 	...
 	if(!arrSaleRate) {
 		flg = false;
 		//$(".log_job").attr('disabled', true);
 		$(".plan_sale_foreign_amt").html(Number(0).toFixed(point));
		$(".plansale_tax").html(Number(0).toFixed(point));
		$(".plan_req_amt").html(Number(0).toFixed(point));
		//$('input.cal-sale-box').next('span').find('input').html("0.0");
 		return flg;
 	}
 	if(objStorage.towlogin == "1"||objStorage.whlogin=="1"){
 		if(objStorage.saleRate==objStorage.saVatRate){
 	 		if(objStorage.saleRate!=arrSaleRate['vat_rate']&&objStorage.saleRate!=null){
 	 			var saleVatRate = objStorage.saleRate;
 	 		}
 	 		else{
 	 			
 	 			var saleVatRate = arrSaleRate['vat_rate'];
 	 			objStorage.saleRate=arrSaleRate['vat_rate'];
 	 		}
 	 		
 	 	}
 	 	else{
 	 		var saleVatRate = arrSaleRate['vat_rate'];
 	 		objStorage.saleRate = saleVatRate;
 	 	}
 		if(objStorage.firstlogin==1){
 			objStorage.firstlogin=0;
 		}
 		else{
 			objStorage.towlogin=0;
 		}
 	...
 	objStorage.newsaVatRate1 = formatNumber(sale_obj['saleBase']);
 	objStorage.newsaVatRate2 = formatNumber(sale_obj['saleVatBase']);
 ...
 function calculationCost(costIsHave) {
 	...
 	if($('#switch-state1').bootstrapSwitch('state')){	
 		if(costCurCode==""){
 			costCurCode = objStorage.hsnumbercost;
 	 	}
 	}
 	...
 	objStorage.newcoVatRate1 = formatNumber(cost_obj['costBase']);
 	objStorage.newcoVatRate2 = formatNumber(cost_obj['costVatBase']);
 ...
 function loopUserCard() {
 	...
 	$('#per .person').each(function(index, element) {
 		...
 		usercd.push($(element).find("div .usercode").text());
 	});
 	var card = [];
 ...
 function jobUpGetNewAddCards() {
 	...
 	$('#per .person').each(function(index, element) {
 		...
 		usercdArr.push($(element).find("div .usercode").text());
 		//}
 	});
 	var cardNew = [];
 	var cardDel = [];
 ...
 function initShow() {
 	...
 	if(nowUrl.match(/\/job_update.html/) != null) {
 		jobcd = getQueryString("jobcd", "jczh/job_update.html");
...
// 			console.log(data);
 			if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
 				$.resetRequestID(data[$.getRequestUserInfoName()].requestID);
 				if(data[$.getRequestDataName()] != null) {
 					objStorage.accountFlg = data[$.getRequestDataName()]['jobland'].accountFlg;
 					objStorage.saVatRate = data[$.getRequestDataName()]['jobland'].sale_vat_rate
 					objStorage.coVatRate = data[$.getRequestDataName()]['jobland'].cost_vat_rate
 					objStorage.firstlogin = "1";
 					objStorage.towlogin = "1";
 					objStorage.whlogin = "1";
 					objStorage.jobEndFlg = data[$.getRequestDataName()]['jobland'].jobEndFlg
 					...
 					if($("#scheduled-date") != undefined) {
 						if(data[$.getRequestDataName()]['jobland'].isSaleFinishFlg==0){
 					...
 					if(jobcd != null) {
   						...
   						objStorage.saleFinishFlg = saleFinishFlg
                        ...
                        objStorage.lockFlg =job['lockFlg'];   
 						...
 						objStorage.enddata = job['plan_dalday'];
 						var sale_id = job['sale_typ'];
						var arrSaleRate = getSaleRate(sale_id);
						objStorage.saleRate = arrSaleRate['vat_rate'];
						objStorage.nodeList = data[$.getRequestDataName()].nodeList;
						 cardInit(arrDelUser,saleFinishFlg);
						 var ifSaleRate = part_language_change_new('IF_SALE_RATE');
						 var ifCostRate = part_language_change_new('IF_COST_RATE');
						 var mdqx = urlPars.parm("md");
						 if(mdqx =="nomdfp"){
							 objStorage.saleRate = objStorage.saVatRate;
							 objStorage.costRate = objStorage.coVatRate;
						}
						 if(saleFinishFlg!=1){
							 if(objStorage.saleRate!=objStorage.saVatRate){
		 							$.messager.confirm('Confirmation', ifSaleRate, function(r) {
		 								if(r) {
		 									if(objStorage.costRate!=objStorage.coVatRate){
		 			 							$.messager.confirm('Confirmation', ifCostRate, function(r) {
		 			 								if(r) {
		 			 									//objStorage.saleRate = objStorage.saVatRate;
		 			 									//objStorage.costRate = objStorage.coVatRate;
		 			 									loadJobMsg(job);
		 			 									objStorage.whlogin = "0";
		 			 								}else{
		 			 									objStorage.costRate = objStorage.coVatRate;
		 			 									//objStorage.saleRate = objStorage.saVatRate;
		 			 									loadJobMsg(job);
...
//                    
 						...
 						$("#plansale_tax_total").text(formatNumber($("#plansale_tax_total").text()));
 						$("#jobrevenue").text(formatNumber($("#jobrevenue").text()));
 						if(!bl && bl2) {
 ...
 function loadJobMsg(job) {
 	...
 	if(Number(job['plan_sale_cure_code'])!=null&&Number(job['plan_sale_cure_code'])!="0.0"){
 		objStorage.hsnumbersale = Number(job['plan_sale_cure_code']);
 	}
 	if(Number(job['plan_cost_cure_code'])!=null&&Number(job['plan_sale_cure_code'])!="0.0"){
 		objStorage.hsnumbercost = Number(job['plan_sale_cure_code']);
 	}
 	//预计终了日
 	...
 	//获取card长度 
 	objStorage.oldcardlength=job.julable.length;
 	//成本
 	//成本外货code
 ...
 if($('#switch-state').bootstrapSwitch('state')){
	 if(objStorage.saleRate == objStorage.saVatRate){
		 if($('#sale_foreign_amt').text()==formatNumber(job['plan_sale'])||$('#sale_foreign_amt').text()=="0.0"){
		 ...
		 else if($('#sale_foreign_amt').text()!=formatNumber(job['plan_sale'])||$('#sale_foreign_amt').text()!="0.0"){
			 $('#sale_foreign_amt').text(formatNumber(job['plan_sale']));
			 $('#sale_tax').text(formatNumber(job['plan_vat_amt']));
			 $('#req_amt').text(formatNumber(job['plan_req_amt']));
		 }
	 ...
	 if(objStorage.newsaVatRate1!="0.0"&& objStorage.newsaVatRate1!=null){
		 if(objStorage.saleRate == objStorage.saVatRate||$('#sale_foreign_amt').text()==0.0){
			 if($('#sale_foreign_amt').text()!=formatNumber(job['plan_sale'])){
				 $('#sale_foreign_amt').text(formatNumber(job['plan_sale']));
			 }else{
				 $('#sale_foreign_amt').text(objStorage.newsaVatRate1);
			 }
		 }
		 else{
			 $('#sale_foreign_amt').text(objStorage.newsaVatRate1);
	 	...
	 	if(objStorage.newsaVatRate2!="0.0"&&objStorage.newsaVatRate2!=null){
	 		 if(objStorage.saleRate == objStorage.saVatRate){
	 			objStorage.old_sale_tax = $('#sale_tax').text();
	 			 if($('#sale_tax').text()!=job['plan_vat_amt']){
					 $('#sale_tax').text(formatNumber(job['plan_vat_amt']));
					
				 }else{
					 $('#sale_tax').text(objStorage.newsaVatRate2);
				 }
	 		 }
	 		 else{
	 			$('#sale_tax').text(objStorage.newsaVatRate2);
	 	...
	 	if(objStorage.saleRate == objStorage.saVatRate){
	 	 ...
	 	 objStorage.oldsale = "1";
		 calculationSale();
		 objStorage.oldsale = "0";
...
//$('.plan_cost_cure_code').val(Number(job['plan_cost_cure_code']));
	if(job['plan_cost_cure_code'] == null || job['plan_cost_cure_code'] == "") {
 	...
 	if(objStorage.newcoVatRate1!="0.0" && objStorage.newcoVatRate1!=null){
 		if(objStorage.costRate==objStorage.coVatRate){
 			if(objStorage.newcoVatRate1!=job['plancostamt']){
 				$('span#cost_foreign_amt').text(formatNumber(job['plancostamt']));
 			}
 			
 		}else{
 			$('span#cost_foreign_amt').text(objStorage.newcoVatRate1);
 		}
 		
 	}
 	else{
 		
 		//$('span#cost_foreign_amt').text(job['plancost_tax'] == null ? Number(0).toFixed(point) : formatNumber(job['plancost_tax']))
 		$('span#cost_foreign_amt').text(job['plancostamt'] == null ? Number(0).toFixed(point) : formatNumber(job['plancostamt']))
 	}
 	
 	if(objStorage.newcoVatRate2!="0.0" && objStorage.newcoVatRate2!=null){
 		
 		if(objStorage.costRate==objStorage.coVatRate){
 			if(objStorage.newcoVatRate2!=job['plancost_tax']){
 				$("#cost_tax").text(formatNumber(job['plancost_tax']));
 				
 			}
 		}else{
 			$("#cost_tax").text(objStorage.newsacotRate2);
 		}
// 		if($("#cost_tax").text()!=job['plancost_tax']){
// 			$("#cost_tax").text(job['plancost_tax']);
// 		}
// 		$("#cost_tax").text(objStorage.newsacotRate2);
 	}
 	else{
 		//$("#cost_tax").text(job['plan_pay_amt'] == null ? Number(0).toFixed(point) : formatNumber(job['plan_pay_amt']));
...
// 	$("#cost_tax").text(job['plancost_tax'] == null ? Number(0).toFixed(point) : formatNumber(job['plancost_tax']))
 	...
 	//税金合计
 	if($('#plansale_tax_total').text()!="0.0"&& $('#plansale_tax_total').text() != null &&$('#plansale_tax_total').text()!=job['plansale_tax_total']){
 		
 		$('#plansale_tax_total').text($('#plansale_tax_total').text());
 	}
 	else{
...
// 	if(objStorage.newsaVatRate1!="0.0"){
// 		$('#sale_foreign_amt').text(objStorage.newsaVatRate1);
// 	}
// 	else{
// 		//预计売上金额
...
// 	}
// 	
// 	if(objStorage.newsaVatRate2!="0.0"){
// 		$('#sale_tax').text(objStorage.newsaVatRate2);
// 	}
// 	else{
// 		//增值锐
...
// 	}
 	...
 	if(objStorage.newcoVatRate1!="0.0" && objStorage.newcoVatRate1!=null  && $('#plansale_tax_total').text()!=job['plansale_tax_total']){
 		var costAtax = floatObj.add(objStorage.newcoVatRate1, $('#plansale_tax_total').text());
 	}
 	else{
 	...
 	if(objStorage.newsaVatRate1!="0.0" && objStorage.newsaVatRate1!=null ){
 		var profit = floatObj.subtract(objStorage.newsaVatRate1, costAtax);
 	}
 	else{
 	...
 	if(objStorage.newsaVatRate1!="0.0" && objStorage.newsaVatRate1!=null){
 		var preProfitRate = floatObj.divide(profit, objStorage.newsaVatRate1);
 	}
 	else{
 	...
 	//
   	if($('#sale_foreign_amt').text()==objStorage.old_sale_tax){
 	...
 	objStorage.oldcost = "1";
	calculationCost()
	objStorage.oldcost = "0";
 	//备考
 ...
 function setUserCard(user_list) {
 	...
 	var md_flg = $("#md-flg").prop("checked");
 	var langTyp =$('#language').val();
 	
 	for(var i = 0; i < user_list.length; i++) {
 		usercode = "jobCreat-usercode-" + parseInt(i + 1);
 		...
 		if(levelValue == 2) {
 			if(mdReqPowerUp()){
 				if(enPromotion(objStorage.nodeList)&&enterprisePromotion(objStorage.nodeList)){
 					str += '<select name="boostrap-select " class="form-control emp" id="' + selectID + '">' +
 	 				'<option name="jobregistration_business_ren" class="i18n hidden" value="2" selected=true >'+part_language_change_new('jobregistration_business_ren')+'</option><option name="jobregistration_bear_ren" class="i18n" value="3" >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>';
 				}else{
 					str += '<select name="boostrap-select " class="form-control emp" id="' + selectID + '">' +
	 				'<option name="jobregistration_business_ren" class="i18n" value="2" selected=true >'+part_language_change_new('jobregistration_business_ren')+'</option><option name="jobregistration_bear_ren" class="i18n" value="3" >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>';
 				}
 				
 			}else{
 				if(enPromotion(objStorage.nodeList)&&enterprisePromotion(objStorage.nodeList)){
 					str += '<select name="boostrap-select " class="form-control emp" id="' + selectID + '">' +
	 				'<option name="jobregistration_business_ren" class="i18n hidden" value="2" selected=true >'+part_language_change_new('jobregistration_business_ren')+'</option><option name="jobregistration_bear_ren" class="i18n hidden" value="3" >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>';
 				}else{
 					str += '<select name="boostrap-select " class="form-control emp" id="' + selectID + '">' +
	 				'<option name="jobregistration_business_ren" class="i18n" value="2" selected=true >'+part_language_change_new('jobregistration_business_ren')+'</option><option name="jobregistration_bear_ren" class="i18n hidden" value="3" >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>';
 				}
 			...
 			if(mdReqPowerUp()){
 				if(enPromotion(objStorage.nodeList)&&enterprisePromotion(objStorage.nodeList)){
 		 			str += '<select name="boostrap-select " class="form-control emp" id="' + selectID + '">' +
 	 				'<option name="jobregistration_business_ren" class="i18n hidden" value="2" >'+part_language_change_new('jobregistration_business_ren')+'</option><option name="jobregistration_bear_ren" class="i18n" value="3"  selected=true >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>';
 				}else{
 					str += '<select name="boostrap-select " class="form-control emp" id="' + selectID + '">' +
 	 				'<option name="jobregistration_business_ren" class="i18n" value="2" >'+part_language_change_new('jobregistration_business_ren')+'</option><option name="jobregistration_bear_ren" class="i18n" value="3"  selected=true >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>';
 				}
 			}else{
 				
 				if(enPromotion(objStorage.nodeList)&&enterprisePromotion(objStorage.nodeList)){
 					str += '<select name="boostrap-select " class="form-control emp" id="' + selectID + '">' +
 					'<option name="jobregistration_business_ren" class="i18n hidden" value="2" >'+part_language_change_new('jobregistration_business_ren')+'</option><option name="jobregistration_bear_ren" class="i18n hidden"  value="3"  selected=true >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>';
 				}else{
 					str += '<select name="boostrap-select " class="form-control emp" id="' + selectID + '">' +
 					'<option name="jobregistration_business_ren" class="i18n" value="2" >'+part_language_change_new('jobregistration_business_ren')+'</option><option name="jobregistration_bear_ren" class="i18n hidden"  value="3"  selected=true >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>';
 				}	
 			}
 		...
 		//不是自己添加的卡片 只要有权限也能删除 （原本不是自己添加的卡片就算有权限也不可以删除） 修改内容 将  close hidden-flg  替换成    close new-card
   		if(saleFinishFlg == 1) {
   			
 				str += '<button type="button" class="close new-card" data-dismiss="modal" aria-hidden="true" id="' + btnId + '"></button>'+
 			'</div></div><div class="person-m"><div class="person-m-l">' +
 			'<i class="iconfont icon-ren cardfind" id="' + colorid + '" style="' + colorStyle + '"></i>' +
 			'</div><div class="person-m-m"><div name="userlevel" id="' + levelid + '">' + cardDeptName(user_list[i]) + '</div>' +
 			'<div class="usercode hidden" id="' + usercode + '">' + user_list[i]["usercd"] + '</div><div class="usercd" id="' + userid + '">' + user_list[i]["memberid"] + '</div><div class="username" id="' + username + '">' + user_list[i]["user_name"] + '</div>'+
 		...
 		if(user_list[i]['level_flg'] == 1) {
 			str += '<div class="person-b"><label><input type="checkbox" class="level_flg" id="'+checkID+'" value="option4" checked="checked">' +
 				'<span class="iconfont icon-duihaok"></span><span class="i18n" name="jobregistration_person_in_charge">'+part_language_change_new('jobregistration_person_in_charge')+'</span></label></div></div>';
 		} else {
...
// 			$("#" + iid).unbind("click");
// 		}
 		if(saleFinishFlg==1 && objStorage.jobEndFlg==0&&mdReqPowerUp()){
 			cardLogic(saleFinishFlg,'mdFlg-click',objStorage.jobEndFlg);	
 		}else{
 			cardLogic(saleFinishFlg,'mdFlg-click');	
 		}
   		//clearCard(selectID, userid, username, levelid);
 		 clearCard(selectID,userid,username,levelid,usercode);
 		//加载语言翻译
//		var insertEle = $(".i18n");
...
//	    });
 	...
 	$(".addp").on("click", function() {
 		...
 		var usercode= "jobCreat-usercode-" + (num + 1);
 		var cardStr ="";
 		var md_flg = $("#md-flg").prop("checked");
 		if(saleFinishFlg==1){
 			...
 			'<option name="jobregistration_bear_ren" class="i18n" value="3" selected="true" >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>' +
 			...
 			'<div class="usercode hidden" id="' + usercode + '"></div><div class="usercd" id="' + userid + '"></div><div class="username" id="' + username + '"></div>' +
 			'</div><div class="person-m-r"><i class="iconfont icon-sousuo" id="' + iid + '"></i></div></div>' +
 			...
 			if(mdReqPowerUp()){
 				if(enPromotion(objStorage.nodeList)&&enterprisePromotion(objStorage.nodeList)){
 					...
 					cardStr+='<div class="col-md-4 panel person" style="padding:5px;margin-bottom:0;width:225px;background:#fff">' +
 		 			'<div class="person-t"><div class="person-t-l" id="' + selectID + '">' +
 		 			'<select name="boostrap-select " class="form-control">' +
 		 			'<option name="jobregistration_business_ren" class="i18n" value="2" >'+part_language_change_new('jobregistration_business_ren')+'</option><option name="jobregistration_bear_ren" class="i18n" value="3" >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>' +
 		 			'</div><div class="person-t-r">' +
 		 			'<button type="button"  class="close new-card" data-dismiss="modal" aria-hidden="true">×</button>' +
 		 			'</div></div><div class="person-m"><div class="person-m-l">' +
 		 			'<i class="iconfont icon-ren" id="' + colorid + '"></i>' +
 				...
 				if(enPromotion(objStorage.nodeList)&&enterprisePromotion(objStorage.nodeList)){
 					cardStr+='<div class="col-md-4 panel person" style="padding:5px;margin-bottom:0;width:225px;background:#fff">' +
 		 			'<div class="person-t"><div class="person-t-l" id="' + selectID + '">' +
 		 			'<select name="boostrap-select " class="form-control">' +
 		 			'<option name="jobregistration_business_ren" class="i18n hidden" value="2" >'+part_language_change_new('jobregistration_business_ren')+'</option><option name="jobregistration_bear_ren" class="i18n hidden" value="3" >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>' +
 		 			'</div><div class="person-t-r">' +
 		 			'<button type="button"  class="close new-card" data-dismiss="modal" aria-hidden="true">×</button>' +
 		 			'</div></div><div class="person-m"><div class="person-m-l">' +
 		 			'<i class="iconfont icon-ren" id="' + colorid + '"></i>' +
 					...
 					cardStr+='<div class="col-md-4 panel person" style="padding:5px;margin-bottom:0;width:225px;background:#fff">' +
 		 			'<div class="person-t"><div class="person-t-l" id="' + selectID + '">' +
 		 			'<select name="boostrap-select " class="form-control">' +
 		 			'<option name="jobregistration_business_ren" class="i18n" value="2" >'+part_language_change_new('jobregistration_business_ren')+'</option><option name="jobregistration_bear_ren" class="i18n hidden" value="3" >'+part_language_change_new('jobregistration_bear_ren')+'</option></select>' +
 		 			'</div><div class="person-t-r">' +
 		 			'<button type="button"  class="close new-card" data-dismiss="modal" aria-hidden="true">×</button>' +
 		 			'</div></div><div class="person-m"><div class="person-m-l">' +
 		 			'<i class="iconfont icon-ren" id="' + colorid + '"></i>' +
 		...
 		if(saleFinishFlg==1 && objStorage.jobEndFlg==0){
 			cardLogic(saleFinishFlg,'mdFlg-click',objStorage.jobEndFlg);	
 		}else{
 			cardLogic(saleFinishFlg,'mdFlg-click');	
 		}
...
//	    });
 ...
 function cardDeptName(user_list){
	 var str ="";
	 var langTyp = $('#language').val();
	 if(langTyp=="jp"){
		 str='<span class="card_jp">'+user_list["itemname_jp"]+'</span>'+'<span class="card_en hidden">'+user_list["itemname_en"]+'</span>'+'<span class="card_zc hidden">'+user_list["itmname"]+'</span>'+'<span class="card_zt hidden">'+user_list["itemname_hk"]+'</span>';
		}else if(langTyp=="zc"){
			 str='<span class="card_jp hidden">'+user_list["itemname_jp"]+'</span>'+'<span class="card_en hidden">'+user_list["itemname_en"]+'</span>'+'<span class="card_zc">'+user_list["itmname"]+'</span>'+'<span class="card_zt hidden">'+user_list["itemname_hk"]+'</span>';
		}else if(langTyp=="zt"){
			 str='<span class="card_jp hidden">'+user_list["itemname_jp"]+'</span>'+'<span class="card_en hidden">'+user_list["itemname_en"]+'</span>'+'<span class="card_zc hidden">'+user_list["itmname"]+'</span>'+'<span class="card_zt">'+user_list["itemname_hk"]+'</span>';
		}else if(langTyp=="en"){
			 str='<span class="card_jp hidden">'+user_list["itemname_jp"]+'</span>'+'<span class="card_en">'+user_list["itemname_en"]+'</span>'+'<span class="card_zc hidden">'+user_list["itmname"]+'</span>'+'<span class="card_zt hidden">'+user_list["itemname_hk"]+'</span>';
		}
	 return str;
 ...
 function logJob() {
	 //控制提示框是否弹出
	 var prompt_box_jurisdiction;
	 prompt_box_jurisdiction = loginJobMD(JSON.parse(parent.localStorage.nodeList));
	 var jobcd = $(".upd-jobcd").text();
	 if(jobcd != null && jobcd != undefined && jobcd != ""){
		 prompt_box_jurisdiction = upJobMD(JSON.parse(parent.localStorage.nodeList))
	 }
	...
	//	$(".saleOp").attr("disabled", true);
 	...
 	if(submitVerify() == "NO_MD" && prompt_box_jurisdiction) {
 		...
 		$.messager.confirm(confirmTitle, msg, function(r) {
 			if(!r) {
 				...
 				job['remark'] = $('.remark').val();
 				 //卖上税率
 				job['sale_vat_rate']=objStorage.saleRate;
 				
 				 //原价税率
 				job['cost_vat_rate']=objStorage.costRate;
...
// 				console.log(job)
 						...
 						if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
 							...
 							if(data[$.getRequestDataName()] != null) {
 								...
 								if(jobcd != null && jobcd != undefined && jobcd != "") {
 									...
 									var url = "jczh/job_registration.html";
 									//					 	if(pdfout){
 		...
 		if(jobcd != null && jobcd != undefined && jobcd != "") {
 			...
 			job['delcard'] = delCard;
 			job['cardnew'] = cardNew;
 			if(objStorage.oldcardlength != card.length || delCard.length!=0){
 				job['jobIsEditMDFlg'] = "jobIsEditMDFlg";
 			}
 			job['lockFlg'] = objStorage.getLockFlg();
 			job['isSaleFinishFlg'] = objStorage.saleFinishFlg;
 		}
 		
 	//  如果是  job更新  并且  如果买上登入后 点击更新会报错E010 
 		var mdqx = urlPars.parm("md");
 		if(objStorage.saleFinishFlg == 1 && mdqx !="nomdfp"){			
 			var url =  "jczh/job_registration_list.html?view=init&menu=se";
				showLockInfoMsgHandler('STATUS_VALIDATEPOWER_ERROR',url);
			return;
 		}
 		...
 		job['assignflg'] = $("#md-flg").prop("checked") ? 1 : 0;
 		 //卖上税率
			job['sale_vat_rate']=objStorage.saleRate;
			 ...
			 //原价税率
			job['cost_vat_rate']=objStorage.getCostRate();
 		//原价完了是否跳过
 		/*
 		var skip = objStorage.getSkip();
 		if(skip.cost == 1) {
 		...
 		*/
 		job['saleVatChangeFlg'] = objStorage.getSaleTaxFlg();
 		job['costVatChangeFlg'] = objStorage.getCostTaxFlg();
 		job['costFinshFlg'] = objStorage.getCostFinishFlg();
 		var cd = {
 			job_land: job
 		}
// 		console.log(job)
 			...
 			success: function(data) {
 				...
 				if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
 					...
 					if(data[$.getRequestDataName()] != null) {
 						...
 						objStorage.saleVatRate = data[$.getRequestDataName()]['jobland'].sale_vat_rate;
 						objStorage.costVatRate = data[$.getRequestDataName()]['jobland'].cost_vat_rate;
 ...
 //select切换 清空
 function clearCard(selectID, userid, username, levelid, usercode) {
 ...
 function loginJobMD(nodeList){
		var flg = false;
		for(var i = 0;i<nodeList.length;i++){
			if(nodeList[i].nodeID==4){
				flg = true;
			}
		}
		return flg;
	}
 function upJobMD(nodeList){
		var flg = false;
		for(var i = 0;i<nodeList.length;i++){
			if(nodeList[i].nodeID==13){
				flg = true;
			}
		}
		return flg;
