...
function clickCllection() {
	...
	$('.sell-num').click(function() {
		window.location.href = $.getJumpPath() + 'jczh/job_registration_list.html?view=job&statusCode=004&dlvday='+AccountDate;
	});
	//到job一览 查询已登录业务未处理job
	$('.not-sell-num').click(function() {
		window.location.href = $.getJumpPath() + 'jczh/job_registration_list.html?view=job&statusCode=002&dlvday='+AccountDate;
	});
	//到job一览 查询 已终止成本未处理件数
	$('.cost-finishnum').click(function() {
		window.location.href = $.getJumpPath() + 'jczh/job_registration_list.html?view=job&costCode=001&dlvday='+AccountDate;
	});
	//到成本一览，成本本月批准成本 flg = 1
	$('.cost-ad').click(function() {
		window.location.href = $.getJumpPath() + 'jczh/cost_list.html?view=0';
	});
	//到成本一览， 成本本月未批准成本 flg = 0
	$('.cost-no-ad').click(function() {
		window.location.href = $.getJumpPath() + 'jczh/cost_list.html?view=1';
	});
	//跳到job一览 根据keyword 模糊检索
	$('.icon-sousuo').click(function() {
		var keyWord = $('#keyword').val();
		window.location.href = $.getJumpPath() + 'jczh/job_registration_list.html?view=top&keyword=' + keyWord;
	});
	...
	$('#mylable li').click(function() {
		...
		window.location.href = $.getJumpPath() + 'jczh/job_registration_list.html?view=job&labelText=' + labletext;
	});
	//我的标签 传参 点击标签的 文本跳到job一览
	...
	for(var i = 0; i < panelList.length; i++) {
		if(panelList[i]['msglevel'] == 1) {
			...
			str += '<div class="newsz">' +
				...
				'<span class="title_label">' + panelList[i]['msgtitle'] + '</span>'+
				'<span> - '+panelList[i]['msgtext'].replace(/&nbsp;|<br\/>|<br>/g, "")+'</span>'+
				'</div>' +
				'<div class="news-r1">' +
...
//	var sum = jobSellRagNum + jobNotSellNum;
    ...
    lange.moneyen = TopObject.getUserInfo().moneyen
    lange.moneyzc = TopObject.getUserInfo().moneyzc
    lange.moneyjp = TopObject.getUserInfo().moneyjp
    lange.moneyzt = TopObject.getUserInfo().moneyzt
    localStorage.setItem('moneyUnit',JSON.stringify(lange));
    setPersonMoneyCode(userData);
