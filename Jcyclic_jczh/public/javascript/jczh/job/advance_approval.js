	  ...
	  headers: {"requestID":$.getRequestID(),"requestName":$.getRequestNameByJcZH()},
			success:function(data){			
	           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
	           	...
	           	if(flag=='1'){
	           		if(data.data.rate != data.data.LendtrnInput.lendtrnList[0].vat_rate){
		           		showErrorHandler("SUBSTITUTE_PAD_COST", "warning", "warning");
		           	}
	        	}
	           	objStorage.personMoneyCode = data["userInfo"];
	           	//获取inputno
	        	objStorage.inputno = data.data.LendtrnInput.lendtrnList[0].input_no;
				//本国货币保留小数位数
...
//				saleIsHave(data.data.LendtrnInput.lendtrnList[0].ISHAVE)//卖上是否税入;0：税拔；1：税入；	
	            ...
	            if(data.data.LendtrnInput.lendtrnList[0].payforeigntype==null||data.data.LendtrnInput.lendtrnList[0].payforeigntype==""){
	            	 $('.plancost_amt').numberbox({
		 	                precision:$.getDefaultPoint()
		 	            });
	            }
	            else{
	            	 $('.plancost_amt').numberbox({
	 	                precision:data.data.LendtrnInput.lendtrnList[0].foreignlen
	 	            });
	            }
	        	...
	        	$('.validatebox-text').attr('disabled','disabled');
	        		taxrate()
...
function advanceApproval(){
	...
	var pars = {}
		pars.lendtrn={
				input_no:input_no,
				old_input_no:objStorage.inputno,
				job_cd:job_cd,
				lend_amt:lend_amt,
...
function setPersonMoneyCode() {
	...
	var toplange = $('#language').val();
	if ($('#switch-state-job').bootstrapSwitch('state') == true){
		var valOther = $("#plan_cost_foreign_code option:selected").text();
	}else {
