...
//	if(saleAddFlag=='0'){
//	  $.layerShowDiv('job_detail','1000', '650',2,'../jczh/job_detail.shtml');
...
//	           		showConfirmMsgHandler("NOTFOUND")
	           	...
	           	if(data.data.PaytrnInput.costtrnList[0].status=='1'||data.data.PaytrnInput.costtrnList[0].status=='2'){
	           		if(data.data.PaytrnInput.paytrnList[0].pay_rate != data.data.PaytrnInput.costtrnList[0].cost_vat_rate){
		           		showErrorHandler("PAY_COST_RATE", "warning", "warning");
		           	}
	           		
	           	}
	           	if(data.data.PaytrnInput.costtrnList[0].status=='0'){
	           		if(data.data.PaytrnInput.costtrnList[0].cost_rate != data.data.PaytrnInput.costtrnList[0].cost_vat_rate){
		           		showErrorHandler("PAY_APPLY_COST", "warning", "warning");
		           	}
	           	...
	           	SelectObj.setLableList = data.data.PaytrnInput.list_lable;
...
//				$('.rmb').text(data.data.PaytrnInput.paytrnList[0].PAYFOREIGNTYPE);
				...
				var payforeigntype = getForgMoneyUnit(data.data.PaytrnInput.foreign,data.data.PaytrnInput.paytrnList[0].pay_foreign_type);
				var costforeigntype = getForgMoneyUnit(data.data.PaytrnInput.foreign,data.data.PaytrnInput.costtrnList[0].cost_foreign_type);
				...
				$("#job_name").text(data.data.CLDIV[0].jobname);//案件名
				...
				$('.payee_cd').text(data.data.CLDIV[0].orderName);//发注先 
				...
				if(pay_auto_setting==1 && flag=='1'){//支付承认+自动设定
					...
					var zfdate = data.data.PaytrnInput.paytrnList[0].adddate;//支付情报登陆日
				...
				if(foreignlen!=null&&foreignlen!=""){
					$('.payforeignamt').text(formatNumber(pointFormatHandler(data.data.PaytrnInput.paytrnList[0].pay_foreign_amt,2,foreignlen),foreignlen));//入力金額
				}else{
					$('.payforeignamt').text(formatNumber(pointFormatHandler(data.data.PaytrnInput.paytrnList[0].pay_foreign_amt,2,foreignlen)));//入力金額
				}
				...
				if(foreignlentwo!=null&&foreignlentwo!=""){
					$('.costforeignamt').text(formatNumber(pointFormatHandler(data.data.PaytrnInput.costtrnList[0].cost_foreign_amt,2,foreignlentwo),foreignlentwo));//入力金額
				}else{
					$('.costforeignamt').text(formatNumber(pointFormatHandler(data.data.PaytrnInput.costtrnList[0].cost_foreign_amt,2,foreignlentwo)));//入力金額
				}
...
//				$('.incoiceno').text(data.data.PaytrnInput.invoiceintrnList[0].INVOICE_NO);//発票番号
				...
				var massageone = data.data.PaytrnInput.paytrnList[0].invoice_type+' '+data.data.PaytrnInput.paytrnList[0].invoice_text;
				var massagetwo = data.data.PaytrnInput.costtrnList[0].invoice_type+' '+data.data.PaytrnInput.costtrnList[0].invoice_text;
