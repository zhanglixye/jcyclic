		...
		success:function(data){
           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
           	...
           	if(data[$.getRequestDataName()] != null)
           	{
           		...
           		if(data[$.getRequestDataName()].invoiceFlg == 0)
           		{
           			$('.point').eq(3).addClass('hidden');
           		}else{
           			$('#invoiceNotRegister').html(data[$.getRequestDataName()].sellInvoiceIsNullConts);
           		}
           	}else{
			...
			$.ajax({
				...
				success:function(data){
		           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
		           	...
		           	{
		           		//结账成功
		           		var path = "jczh/top_registration.html";
		           		showInfoMsgHandler("EXECUTE_SUCCESS","SUCCESS","info",path,"");
		           	}
		           	else if(data[$.getRequestDataName()]!=null && data[$.getRequestDataName()]!="" && data[$.getRequestDataName()]!="0" && data[$.getRequestDataName()]!="1"){
		           		var reg=new RegExp("-","g");
		           		var stringObj=data[$.getRequestDataName()]; 
		           		var newstr=stringObj.replace(reg,"\n"); 
		           		showErrorHandler("APPROVAL_SALE_RATE", "warning", "warning");
		           		$('div.panel.window.messager-window div.messager-icon.messager-warning').next().text($('div.panel.window.messager-window div.messager-icon.messager-warning').next().text() + newstr)
		           	}
...
function searchJobListByMonthCheckOut(pageFlg,parName,statusCode)
{
	...
	if(pageFlg == "job")
	{
		path += "jczh/job_registration_list.html?backPage=checkOut&view=";
		path += pageFlg+"&"+parName+"="+statusCode;
	}else{
		path += "jczh/cost_list.html?backPage=checkOut&view="+statusCode+"&dalday=dalday1";
		//path += "jczh/cost_list.html?backPage=checkOut&view="+statusCode;
	}
	location.href = path;
}
objStorage = {
