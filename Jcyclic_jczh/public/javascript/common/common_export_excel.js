 ...
 * @date 日期 2018.05.18
 */
function exportExcelByTabOld(tabID, excelName) {
	if(getExplorer() == 'ie') {
		//获取Datagride的所有数据集合
		...
		for(var j = 0; j < nameList.length; ++j) {
			...
			if(nameList[j].align != 'undefined' && nameList[j].align != '') {
				if(nameList[j].field == "saleamt" || nameList[j].field == "vatamt" || nameList[j].field == "reqamt" ||
					...
					nameList[j].field == "payamt" || nameList[j].field == "amt" || nameList[j].field == "plansale" || nameList[j].field == 'tax2') {
					if ($.getDefaultPoint() == '2'){
						...
						tableString += ' style="vnd.ms-excel.numberformat:#,##0.0;text-align:' + nameList[j].align + ';"';
					}else {
						tableString += ' style="vnd.ms-excel.numberformat:#,##0;text-align:' + nameList[j].align + ';"';
					}
				}else if (nameList[j].field == 'client_flg' || nameList[j].field == 'contra_flg' || nameList[j].field == 'pay_flg' || nameList[j].field == 'hdy_flg'){
...
function OutPutPdfHandler(jobNo, costNo, inputNo, fName, jumpflg, str, jobDtFlg, listFlg) {
	var pdfName = "InvoiceRequest,ReceiptRequest"
			...
			if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
				...
				if(data[$.getRequestDataName()] != null) {
					...
					if(str!="invoiceApplication"){
						...
						var newDate =data.userInfo.dateCompanyZone;
						var dateStr = newDate.replace(/-/g,"");
						dateStr = dateStr.replace(/:/g,"");
						dateStr = dateStr.replace(/ /g,"");
						downLoadPdf(data[$.getRequestDataName()], "Invoice&Receipt_"+dateStr.substring(2,14), "zip");
					}
						...
						if(jumpflg != "1") {
							...
							if(fName == "invoiceApplication"&&jobDtFlg != "jobDetails") {
								...
								indexArr.push(rowIndex);
									...
									var rowAll = $('#cost_list_Two').datagrid("getData")['originalRows'];
								var total = rowAll.length;
								if(total == 0) {
									$('.switch_table').css('display', 'none');
									$('.switch_table + .switch_table_none').removeClass('hidden');
...
function OutPutPdfHandlerMore(jobNo, costNo, inputNo, fName, jumpflg, str, jobDtFlg) {
	var langT = localStorage.getItem('language');
	var pdfName = getFileNameByExcel(fName);
	if(costNo==""){
		var pdfInfo = {
				jobList: jobNo,
				costNo: costNo,
		...
		success: function(data) {
			if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
				...
				if(data[$.getRequestDataName()] != null) {
					...
					dateStr = dateStr.replace(/:/g,"");
					dateStr = dateStr.replace(/ /g,"");
					downLoadPdf(data[$.getRequestDataName()], getFileNameByExcel(fName)+"_"+dateStr.substring(2,14), "zip");
...
//					}
					...
					if(jumpflg != "1") {
						...
						if(fName == "invoiceApplication") {
							...
							 
							for(var i = 0; i < rows.length; i++) {
								var rowIndex = $('#job_login_list').datagrid('getRowIndex', rows[i]);
								if($("#invpd").val() == '003') {
									$('#job_login_list').datagrid('deleteRow', rowIndex);
								} else {
									$('#job_login_list').datagrid('updateRow', {
...
function exportExcelReport(fName, endDate,startDate , cldivID,accountCd) {
	var sheetName = getSheetName(fName);
	var langT = localStorage.getItem('language');
		...
		success: function(data) {
			...
			if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
				...
				if(data[$.getRequestDataName()] != null) {
					...
					}else if (data[$.getRequestDataName()] == "DATA_IS_NOT_EXIST"){
						//showErrorHandler("DATA_IS_NOT_EXIST", "ERROR", "ERROR");
					...
					}else if(data[$.getRequestDataName()] == "CLMST_RANGE_ERROR"){
						 showErrorHandler('CLMST_RANGE_ERROR','info','info');	
						return ;
					}else {
						var fileName = getFileNameByExcel(fName);
						if(fileName == "accountReports")
						{
							for(var i = 0;i < data[$.getRequestDataName()].length;i++)
							{
								/*
								if(i == 0)
								{
									fileName = "accountingEntries";
								}else if(i == 1)
								{
									fileName = "accountingEntriesAccountPayable";
								}else if(i == 2)
								{
									fileName = "accountingEntry";
								}else{
									fileName = "accountingConfirm";
								}
								*/
								fileName = "MonthlyJournalEntries";
								downLoadPdf(data[$.getRequestDataName()][i], fileName, "xlsx");
							}
						}else{
...
function getFileNameByExcel(fName)
{
	...
	switch (fName){
		...
		case "predestineCostList":
			...
			break;
		case "accountReports":
			fileName = "accountReports";
			break;
		case "paybelReport":
			fileName = "AccountsPayableReport";
			break;
		case "madeCostList":
			fileName = "Work-in-progressReport";
			break;
		//pdf
		case "jobCreate":
			fileName = "JobEntry";
...
function getSheetName(fName){
	var sheetName = null;
	switch(fName){
		case "employeesPowerList":
			sheetName = "StaffAccessSecurity";
			break;
		case "jobList":
			sheetName = "JobStatusList";
			break;
		case "jobLabelsReport":
			sheetName = "JobStatusList(Label)";
			break;
		case "monthBalance":
			sheetName = "SalesReport";
			break;
		case "predestineCostList":
			sheetName = "EstimatedCostReport";
			break;
		case "accountReports":
			sheetName = "MonthlyJournalEntries";
			break;
		case "paybelReport":
			sheetName = "AccountsPayableReport";
			break;
		case "madeCostList":
			sheetName = "Work-in-progressReport";
			break;
		}
		return sheetName;
}
