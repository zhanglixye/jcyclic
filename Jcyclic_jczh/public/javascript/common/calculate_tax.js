...
function pointFormatHandler(amt,calFlg,point)
{
	...
	switch (calFlg){
		...
		case 1:
			...
			moneyAmt = floatObj.divide(Math.ceil(floatObj.multiply(Math.abs(moneyAmt) , cr)) , cr);
			moneyAmt = floatObj.multiply(aSymbol , moneyAmt);
			break;
			//切下
		case 2:
			//amt =floatObj.divide(floatObj.multiply(moneyAmt,100),100);
			moneyAmt = floatObj.divide(Math.floor(floatObj.multiply(Math.abs(moneyAmt) , cr)) , cr);
			moneyAmt = floatObj.multiply(aSymbol , moneyAmt);
			break;
			//45入
		case 3:
			//moneyAmt = floatObj.divide(Math.round(floatObj.multiply(moneyAmt , cr)) , cr);
			moneyAmt = floatObj.divide(floatObj.multiply(moneyAmt , cr).toFixed(2) , cr);
			break;
	}
	return formatNumber(moneyAmt,point,false);
}
