...
function settingPageStyle() {
	obj = {
		pageDefault: function(str,tabId) {
			...
			if($("#label_management").exist()){
				autoSolvePage50('label_management',arr_info_textNew);
			}else if(tabId == 'wedgemembersleft'){
				autoSolvePage50('wedgemembersleft',arr_info_textNew);
			}else if(tabId == 'irrelevantTab'){
				autoSolvePage50('irrelevantTab',arr_info_textNew);
			}else if(tabId == 'wedgemembersright'){
				autoSolvePage50('wedgemembersright',arr_info_textNew);
			}else if(tabId == 'relevancedTab'){
				autoSolvePage50('relevancedTab',arr_info_textNew);
			}
...
function autoSolvePage50(tableId,arr_info_textNew) {
	...
	if (pageSize == 51){
		...
		if (arr_info_textNew[2] == arr_info_textNew[1] && arr_info_textNew[2] == arr_info_textNew[0]){
			arr_info_textNew[0] = 0;
			arr_info_textNew[1] = 0;
		}
		if (arr_info_textNew[1] == arr_info_textNew[2]){
			arr_info_textNew[1] = Number(arr_info_textNew[1])-1;
		}
...
function initDataGridHandler(tabId, pageSizeJ, sortNameJ, pagePositionJ, paginationJ, isHasFn, dataColumns, fitColumn,frozen) {
	if( tabId != "cost_list_Two" && tabId != "job_login_list" && tabId != "timesheet_list" )
	{
		...
		if(paginationJ)
		{
			if(tabId == 'wedgemembersleft' || tabId == 'wedgemembersright' || tabId == 'relevancedTab' || tabId == 'irrelevantTab' || tabId == 'label_management'){
				pageSizeJ = 50;
			}else {
				pageSizeJ = 50;
			}
	...
	if(frozen){
		if(dataColumns.length > 0){
			...
			if(tabId == 'job_login_list'){
				var date_type_frozen_arr = ["dlvday","jsflaglanguagejp","jsflaglanguagezc","jsflaglanguagezt","jsflaglanguageen","assignflglanguagejp","reqflglanguagejp","recflglanguagejp","invflglanguagejp","costendflglanguagezc","adddate","saleadddate","saleadmitdate","dlvmon"];
			...
			if(tabId == 'cost_list_Two'){
				var date_type_frozen_arr = ["dalday","statusjp","statuszc","statuszt","statusen","adddate","payreqdate","confirmdate","outputflgjp","outputflgzc","outputflgzt","outputflgen","payflgjp","invoicedate","paydlyday","plandlvday","payhopedate","planpaydate","paydate","lenddate"];
			...
			if (tabId == 'timesheet_list'){
				var date_type_frozen_arr = ["dlvday","jsflaglanguagejp","jsflaglanguagezc","jsflaglanguagezt","jsflaglanguageen","assignflglanguagejp","assignflglanguageen","assignflglanguagezc","assignflglanguagezt","reqflglanguagejp","reqflglanguageen","reqflglanguagezc","reqflglanguagezt","recflglanguagejp","recflglanguagezt","recflglanguageen","recflglanguagezc","invflglanguagejp","invflglanguageen","invflglanguagezc","invflglanguagezt","costendflglanguagezc","costendflglanguagejp","costendflglanguagezt","costendflglanguageen","adddate","saleadddate","saleadmitdate","dlvmon"];
				var cost_frozen_arr = ["saleamt","vatamt","reqamt","plancostamt","costTotalAmt","costVatTotal","tax2","tax3","taxTotal","profit","profitRate","plansale"];
				if(date_type_frozen_arr.indexOf(deleteArr[0].field) != -1){
					deleteArr[0].align = 'center';
				}else if(cost_frozen_arr.indexOf(deleteArr[0].field) != -1){
					deleteArr[0].align = 'right';
			...
			for(var k = 0;k < deleteArr.length;k++){
				if(deleteArr[k]['field']=='jsflaglanguagezc' || deleteArr[k]['field']=='statuszc'){
					deleteArr[k]['field'] = deleteArr[k]['field'].substr(0,deleteArr[k]['field'].length-2)+$.getLangTyp();
				}
			}
	...
	tabIdDom.datagrid({
		...
		onSortColumn: function (sort, order) {
            if(tabId == 'label_management'){
				...
				$('#'+sort).val(window.search_input_value);
            ...
            if(tabId == 'wedgemembersleft' || tabId == 'wedgemembersright'){
				...
				if (tabId == 'wedgemembersleft'){
					$('#'+sort+'L').val(window.search_input_value);
				}else {
					$('#'+sort+'R').val(window.search_input_value);
				}
            	//排序问题第二阶段逆序删除插入操作
            	window.label_ma_order= order;
			...
			if(tabId == "job_login_list"){
	          ...
	          if(rowDataAll.length > 1){
				...
				$('#menu2').menu('show', {  
	          	...
	          	var menu_height = $('#menu').height() + 6;
	          	if (rowDataAll[0].jobInvNo != null && rowDataAll[0].jobInvNo != ""){
					$('#createInvoiceNo').attr('name','joblist_menu_update');
					$('#createInvoiceNo').text(part_language_change_new('joblist_menu_update'));
					$('#popUpTitle').attr('name','joblist_menu_update');
					$('#popUpTitle').text(part_language_change_new('joblist_menu_update'));
					$('#invoiceNo').val(rowDataAll[0].jobInvNo);
					$('#createInvoiceBtn').attr('name','salescategoryList_change');
					$('#createInvoiceBtn').text(part_language_change_new('salescategoryList_change'));
					$('#deleteBtnInput').removeClass('hidden');
				}else {
					$('#createInvoiceNo').attr('name','joblist_menu');
					$('#createInvoiceNo').text(part_language_change_new('joblist_menu'));
					$('#popUpTitle').attr('name','joblist_menu');
					$('#popUpTitle').text(part_language_change_new('joblist_menu'));
					$('#invoiceNo').val("");
					$('#createInvoiceBtn').attr('name','salescategoryRegistration_login');
					$('#createInvoiceBtn').text(part_language_change_new('salescategoryRegistration_login'));
					$('#deleteBtnInput').addClass('hidden');
				}
	          	//选中单条的时候
	          	$('#menu').menu('show', {  
	               //显示右键菜单  
		...
		case "irrelevantTab":
			arr.push({
				...
				sorter:function (a,b) {				
					...
					if(b == c){
						if($('#'+tabId).datagrid('options').sortOrder == 'desc'){
							return -1;
						}else {
							return 1;
						}
...
//			);
		...
		case "clientPopTab":
			...
			arr.push({
				field: 'cldivcd',
				title: part_language_change_new('customlist_list_of_items_title-23-new'),
				width: '30%',
				align: 'center',
				hidden: true
			}, 
...
function formatListTitle(arr,languageFlg){
	...
	for(var i=0;i<arr.length;i++){
		if(arr[i].field == "saleamt" || arr[i].field == "vatamt" || arr[i].field == "reqamt" || arr[i].field == "plancostamt" || arr[i].field == "costTotalAmt" || arr[i].field == "costVatTotal" || arr[i].field == "tax2" || arr[i].field == "tax3" || arr[i].field == "taxTotal" || arr[i].field == "profit"|| arr[i].field == "amt"|| arr[i].field == "payamt" || arr[i].field == 'plansale'){
		...
		&&arr[i]['field']!='invflglanguagezc'&&arr[i]['field']!='costendflglanguagezc'&&arr[i]['field']!='assignflglanguagezc'&&arr[i]['field']!='tranitemnamezc'&&arr[i]['field']!='lendnamezc'){
			newList.push(arr[i]);
		}else{			
...
function addTableSort(columnArr,tabId){
	...
	for(var i = 0;i < arrLen;i ++){
		...
		if(tabId == 'cost_list_Two'){
			date_type_arr = ["disjp","disen","diszc","diszt","dalday","statusjp","statusen","statuszc","statuszt","adddate","payreqdate","confirmdate","outputflgjp","outputflgen","outputflgzt","outputflgzc","payflgjp","payflgzc","payflgzt","payflgen","invoicedate","paydlyday","plandlvday","payhopedate","planpaydate","paydate","lenddate"];
		...
		if(tabId == 'timesheet_list'){
			date_type_arr = ["dlvday","jsflaglanguagejp","jsflaglanguagezc","jsflaglanguagezt","jsflaglanguageen","assignflglanguagejp","assignflglanguageen","assignflglanguagezc","assignflglanguagezt","reqflglanguagejp","reqflglanguageen","reqflglanguagezc","reqflglanguagezt","recflglanguagejp","recflglanguagezt","recflglanguageen","recflglanguagezc","invflglanguagejp","invflglanguageen","invflglanguagezc","invflglanguagezt","costendflglanguagezc","costendflglanguagejp","costendflglanguagezt","costendflglanguageen","adddate","saleadddate","saleadmitdate","dlvmon"];
...
function setDomNameByLanguage(tabId) {
	switch(tabId) {
		...
		case "commonListTab":
			...
			dom.eq(7).attr("name", "commoncodeList_numbers");
			dom.eq(8).attr("name", "commoncodeList_auxiliary_code");
...
function menuInit(){
	 ...
	 var jobInvNo=rowDataAll['jobInvNo'];
	 var jobNo = rowDataAll['jobcd'];
	 ...
	 $('#createInvoiceNo').removeClass('hidden');
	...
	if( isHavePower(menuNode,[18,19,20,21])==false){
	  	...
	  	if(account_flg=='1' || (jobInvNo != "" && jobInvNo != null)){//如果计上济了，则不可以壳上更新，或登录
	  		 $('#saleEdit').addClass('hidden');
	         $('#saleUpdate').addClass('hidden');
		}else{
...
//		      }
	 ...
	 if( isHavePower(menuNode,[22])==false){
	  	  ...
	  	  if(account_flg=='1'||saleaddflg=='0' || (jobInvNo != "" && jobInvNo != null)){//壳上登录/更新
	       $('#saleCancel').addClass('hidden');
		  }else{
	  ...
	  //发票编号登陆
	  if( isHavePower(menuNode,[92])==false){
	  	     $('#createInvoiceNo').addClass('hidden');
	  }else{
		  var invoStatus = getInvoiceStatus(jobNo);
		  if(invoStatus == "isShow")
		  {
			  if(saleaddflg=='0' || account_flg=='1'){//壳上未登录
				  $('#createInvoiceNo').addClass('hidden');
			  }else{
				  $('#createInvoiceNo').removeClass('hidden');
			  } 
...
function menuInitMore(){
	...
	for(var i =0;i<rowDataAll.length;i++){
		/**
		 * 如果有任何一条数据不能做入金处理（売上为承认）则 不会显示出入金处理的菜单
		 * @author作者 项勇
		 * @date 日期 2019.12.24
		 */
		if(rowDataAll[i]['saleadmitflg']=='0'){
			saleadmitflg = 0
		}
	...
	if(recflgnum==rowDataAll.length){
		...
		if( isHavePower(menuNode,[35])==false){
			 ...
			 if(skip['confirm']==0){
			    ...
			    //if(saleaddflg=='0'){//壳上未登录
	  		 if(saleaddflgnum!=rowDataAll.length){
			       $('#rujinM').addClass('hidden');
			    }else{
			       $('#rujinM').removeClass('hidden');
			    }
	...
	/*
	 //发票编号登陆
	  if( isHavePower(menuNode,[92])==false){
		  ...
		  if(saleaddflgnum!=rowDataAll.length){//壳上未登录
			  $('#createInvoiceNo').addClass('hidden');
		  }else{
			  $('#createInvoiceNo').removeClass('hidden');
		  }
...
function menuLoad() {
	...
	$("#menu2").find(".cost-out").removeClass("hidden");
	...
	if(rows.length > 1) { //选中了多条
	   var numT = 0;
		...
		for(var i = 0; i < rows.length; i++) {
			//有一条不是支付原价  菜单中不显示支付处理按钮 并且  不能打印发注书
			if(rows[i].discd != "001") {
				$("#menu2").find(".pay_handle2").addClass("hidden");
				$("#menu2").find(".cost-out").addClass("hidden");
				return ;
			}else{
		...
		if(row.discd == "001") {
			...
			if(row.statuscd==4){
				...
				//2020/07/16 由于考虑数据移情 时可能会有问题数据  解开隐藏发注更新的注释   宗哥确认
...
function getInvoiceStatus(jobNo)
{
	var pars = {};
	pars['searchFlg']="checkInvoiceStatus";
	pars['job_cd']=jobNo;
	...
	$.ajax({
		...
		success:function(data){
			if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
           		...
           		invStatus = data[$.getRequestDataName()];
			}else{
