...
function initform(){
		...
		success:function(data){
           ...
           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
			   ...
			   if (wedgemembersleft.total > 1){
				   $('#wedgemembersleft').datagrid('freezeRow',0);
			   }else {
				   $('#wedgemembersleft').datagrid('freezeRow',-1);
			   }
				$.rowleft = rowleft;
			   ...
			   if (wedgemembersright.total > 1){
				   $('#wedgemembersright').datagrid('freezeRow',0);
			   }else {
				   $('#wedgemembersright').datagrid('freezeRow',-1);
			   }
				$.rowright = rowright;
...
//	var id = this.id;
	...
	if(gridname=='wedgemembersleft'){
		//分页检索保证在第一页显示
		if ($('#wedgemembersleft').datagrid('options').pageNumber > 1){
		...
		if ($('#wedgemembersright').datagrid('options').pageNumber > 1){
