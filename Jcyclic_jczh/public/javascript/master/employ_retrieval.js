			...
			if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
				...
				if(data[$.getRequestDataName()] != null) {
					...
					parent.$('#per .person').each(function(index, element) {
						if($(element).find("div .usercode").text() != '' && $(element).find("div .usercode").text() != null && $(element).find("div .usercode").text() != undefined) {
					...
					if(userData.length >= 0 && $('#pd').val() != 'searchCus'){
						for(var i = 0; i < departData.length; i++) {
							if(departData[i].itemcd == departId)
								{
									dArr.push(departData[i]);
...
//                  addCard(thisDom,2);
					if(langTyp=="jp"){
						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_jp">'+rows[i]['departNamejp']+'</span>'+'<span class="card_en hidden">'+rows[i]['departNameen']+'</span>'+'<span class="card_zc hidden">'+rows[i]['departNamezc']+'</span>'+'<span class="card_zt hidden">'+rows[i]['departNamezt']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s hidden">'+rows[i]['departNameen']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s hidden">'+rows[i]['departNamezc']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s hidden">'+rows[i]['departNamezt']+'</span>');
					}else if(langTyp=="zc"){
						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_jp hidden">'+rows[i]['departNamejp']+'</span>'+'<span class="card_en hidden">'+rows[i]['departNameen']+'</span>'+'<span class="card_zc">'+rows[i]['departNamezc']+'</span>'+'<span class="card_zt hidden">'+rows[i]['departNamezt']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s hidden">'+rows[i]['departNameen']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s">'+rows[i]['departNamezc']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s hidden">'+rows[i]['departNamezt']+'</span>');
					}else if(langTyp=="zt"){
						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_jp hidden">'+rows[i]['departNamejp']+'</span>'+'<span class="card_en hidden">'+rows[i]['departNameen']+'</span>'+'<span class="card_zc hidden">'+rows[i]['departNamezc']+'</span>'+'<span class="card_zt">'+rows[i]['departNamezt']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s hidden">'+rows[i]['departNameen']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s hidden">'+rows[i]['departNamezc']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s">'+rows[i]['departNamezt']+'</span>');
					}else if(langTyp=="en"){
						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_jp hidden">'+rows[i]['departName' + jp]+'</span>'+'<span class="card_en">'+rows[i]['departNameen']+'</span>'+'<span class="card_zc hidden">'+rows[i]['departNamezc']+'</span>'+'<span class="card_zt hidden">'+rows[i]['departNamezt']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s">'+rows[i]['departNameen']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s hidden">'+rows[i]['departName' + zc]+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s hidden">'+rows[i]['departName' + zt]+'</span>');
					}
					//parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).text(rows[i]['departName' + langTyp]);
				    parent.$('#jobCreat-usercd-' + (parseInt(flgs[2])+i)).text(rows[i]['memberID']);
				    ...
				    parent.$('#jobCreat-usercode-' + (parseInt(flgs[2])+i)).text(rows[i]['userCD']);
				}else{
					...
					arrDelUser.push(parent.$('#jobCreat-usercode-' + flgs[2]).text());
					}
...
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s">'+rows[i]['departNamezt']+'</span>');
						...
						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_jp hidden">'+rows[i]['departNamejp']+'</span>'+'<span class="card_en">'+rows[i]['departNameen']+'</span>'+'<span class="card_zc hidden">'+rows[i]['departNamezc']+'</span>'+'<span class="card_zt hidden">'+rows[i]['departNamezt']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s">'+rows[i]['departNameen']+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s hidden">'+rows[i]['departName' + zc]+'</span>');
//						parent.$('#jobCreat-userlevel-' + (parseInt(flgs[2])+i)).html('<span class="card_s hidden">'+rows[i]['departName' + zt]+'</span>');
					}
				    //parent.$('#jobCreat-userlevel-' + flgs[2]).text(rows[i]['departName' + langTyp]);
					parent.$('#jobCreat-usercd-' + flgs[2]).text(rows[i]['memberID']);
					parent.$('#jobCreat-username-' + flgs[2]).text(rows[i]['nickname']);
					parent.$('#person-color-' + flgs[2]).css('color',rows[i]['usercolor']);
					parent.$('#jobCreat-usercode-' + flgs[2]).text(rows[i]['userCD']);
				}
...
//		if(rows[0].level == '001') {
			var name =departname + "|" + rows[0]['memberID'] + "|" + rows[0]['nickname'];
//		} 
