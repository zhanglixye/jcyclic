 ...
 *
 * 10171 - 10175 //重新赋值data.rows数组
 * 10155 - 10157 //data.originalRows深拷贝
 * 795 - 798 11209 - 11213 table表格拖拽隐藏bug
 */
(function($){
...
//暂时return -- chrome下载文件弹出 触发resize
return;
$("body").panel("doLayout");
}
_261=null;
...
var dlg=_2c5(opts);
/************************增加窗口关闭触发事件*****************************/
var dlg_prev = dlg.prev().find('.panel-tool a.panel-tool-close');
dlg_prev.on('click',function(){
	if (fn) {
        fn();
    }
})
/************************end*****************************/
return dlg;
},confirm:function(_2ca,msg,fn,flag_z){
var class_z = (flag_z == 1)?'pop_z':'';
var opts=typeof _2ca=="object"?_2ca:{title:_2ca,msg:msg,fn:fn};
opts=$.extend({},$.messager.defaults,{content:"<div class=\""+class_z+" messager-icon messager-question\"></div>"+"<div>"+opts.msg+"</div>"+"<div style=\"clear:both;\"/>"},opts);
if(!opts.buttons){
opts.buttons=[{text:opts.ok,onClick:function(){
_2c0(dlg,true);
...
//自己加的可删除
window.total_time;
if((window.total_time != 'defined') && (window.total_time == true)){
view.height(_66d);
}
};
...
//data.originalRows深拷贝
if (data.originalRows == undefined){
    var zlxArray = data.rows.clone();
}else {
    var zlxArray = data.originalRows.clone();
}
zlxArray.sort(function(r1,r2){
window.cost_sort = true;
var r=0;
for(var i=0;i<_729.length;i++){
var sn=_729[i];
var so=_72a[i];
var col=_6a4(_727,sn);
// added 点击sorter 切换语言后col为null的判断
if (col === null){
    var _72b=function(a,b){
...
//重新赋值data.rows数组
var startZlx = (opts.pageNumber - 1) * parseInt(opts.pageSize);
var endZlx = startZlx + parseInt(opts.pageSize);
data.rows = zlxArray.slice(startZlx, endZlx);
if (data.originalRows != undefined){
...
window.datagridPageNumber = null;
if(opts.pageNumber!=_72d.pageNumber&&_72d.pageNumber>0){
window.datagridPageNumber=opts.pageNumber;
...
if(col.formatter){
	if(_83e != "lable"){
		cc.push(col.formatter(escapeHtml(_83f),_83c,_83b));
	}else{
		cc.push(col.formatter(_83f,_83c,_83b));
	}
}else{
	if(_83e != "lable"){
		cc.push(escapeHtml(_83f));
	}else{
		cc.push(_83f);
	}
}
