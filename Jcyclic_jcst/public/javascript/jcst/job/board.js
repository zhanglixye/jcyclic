		  ...
		  success:function(data){
		        $.resetRequestID(data[$.getRequestUserInfoName()].requestID);
		           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
					...
					setContent(headerText,bodyText);
					//点击修改，修改那快的按钮从登陆变成修改
...
function setContent(headerText,bodyText){
    UM.getEditor('myEditor').setContent(bodyText);
	        ...
	        $.resetRequestID(data[$.getRequestUserInfoName()].requestID);
	           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
	           	if(data.data==-1){//lockflg验证
		           	$.resetRequestID(data[$.getRequestUserInfoName()].requestID);
		           	showLockInfoMsgHandler("STATUS_VALIDATEPOWER_ERROR","/jcst/top_registration.html");
		           	return false;
		        }
	           	if(data[$.getRequestDataName()]=="NOCHANGE"){
	           		showErrorHandler("SYS_VALIDATEPOWER_ERROR","info","info");
