function initShow() {
	...
	if(nowUrl.match(/\/salescategory_update.html/) != null) {
		jobcd = getQueryString("jobcd", "jcst/salescategory_update.html");
	}
	if(nowUrl.match(/\/salescategory_registration.html/) != null) {
		jobcd = getQueryString("jobcd", "jcst/salescategory_registration.html");
	}
	...
	for(var i = 0; i < card_list.length; i++) {
     var colorStyle = "color:"+card_list[i]['usercolor'];
		...
		if(card_list[i]['level_flg'] == 1 || card_list[i]['level_flg'] == 2) {
			str += '<span class="text-center block">营业担当者</span>';
		} else {
			str += '<span class="text-center block">割当担当者</span>';
		}
		...
		str += '</div>' +
			...
			card_list[i]["itmname"] +
			'</div>' +
			'<div class="usercd">' + card_list[i]["usercd"] + '</div>' +
			'<div class="username">' +
