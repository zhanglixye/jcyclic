           ...
           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
           	$.resetRequestID(data[$.getRequestUserInfoName()].requestID);
           		...
           		if(idlist==undefined||idlist==null||idlist.length<1){
           		 //url路径修改
					if(jobdetailflag=='joblist'){
						url = "jcst/job_registration_list.html?view=init&menu=se";
					}else if(jobdetailflag=='cost'||jobdetailflag=='tran'||jobdetailflag=='lend'||jobdetailflag=='cost'){
				 	    url = "jcst/cost_list.html?view=6";
				 	}else {
	...
	$.ajax({
		...
		success:function(data){
		 ...
		 /*       for(var i=0;i<foreign.length;i++){
		        ...
		        $("#profit").text(formatNumber(truecon['profit']));		    //实际营业收益    
		        $("#profitRate").text(truecon['profitRate']);		 //实际营业收益率	 
		        }
			    ...
			    if(saleInfo["reqamt"]==undefined||saleInfo["reqamt"]==null||saleInfo["reqamt"]===''||jobinfo["planreqamt"]==undefined||jobinfo["planreqamt"]==null||jobinfo["planreqamt"]===''){
				 ...
				 $("#plansaleforeignamt").text(formatNumber(jobinfo["plansaleforeignamt"]));//预计金额		        
		       ...
		       //壳上承认bu跳过 
				...
				for(var i = 0; i < JobUserList.length;i++){
                        if(JobUserList[i]["level_flg"]=='1'){
								...
								+'<div class="person-t"><span class="text-center block i18n" name="job_yingye" ></span></div>'
								+'<div class="person-m"><div class="person-m-l"><i class="iconfont icon-ren" style="color:'+JobUserList[i]["usercolor"]+';"></i></div>'
							    +'<div class="person-m-m"><div class="userlevel">' + JobUserList[i]["itmname"] + '</div>'
								+'<div class="usercd">' + JobUserList[i]["usercd"] + '</div>'
								...
								+'<div class="person-t"><span class="text-center block i18n" name="job_gedang" ></span></div>'
								+'<div class="person-m"><div class="person-m-l"><i class="iconfont icon-ren" style="color:'+JobUserList[i]["usercolor"]+';"></i></div>'
							    +'<div class="person-m-m"><div class="userlevel">' + JobUserList[i]["itmname"] + '</div>'
								+'<div class="usercd">' + JobUserList[i]["usercd"] + '</div>'
								+'<div class="username">' + JobUserList[i]["user_name"] + '</div></div></div><div class="person-b"><label>'
           ...
           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
 		    ...
 		    // 类似入力共同区域 cost --star 
 		     ...
 		     if(setJobdetailForeignMoneyCode(foreign,'itemname',costList['costforeigntype'])['point']==''){
 		      ...
 		      $('.costishave').find('.active').find('input').each(function(index,element){
           ...
           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
		      ...
		      /*  外貨赋值*/
	           ...
	           $('.lendishave').find('.activeL').find('input').each(function(index,element){
                ...
                if(costList["costfinishflg"]=="1"){
		            //原价终了
		             $('#costflagimg').attr('src', '../public/images/job3.4.png');
		        }
          ...
          if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
 		     ...
 		     $.resetRequestID(data[$.getRequestUserInfoName()].requestID);
 			  ...
 			  if(data[$.getRequestDataName()]['TrantrnInput']['lableList'].length>0){
 			  	var TranlableList =data[$.getRequestDataName()]['TrantrnInput']['list_lable'];
	           ...
	           if(trantrnDate['confirmdate']==null||trantrnDate['confirmdate']==''){
 			   ...
 			   //状态位图片
 			    if(costList["costfinishflg"]=="1"){
		            //原价终了
		             $('#costflagimg').attr('src', '../public/images/job4.3.png');
	...
	$.ajax({
		...
		success: function(data) {
			if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
				...
				initDataGridHandler("cost_list_job_details", null, null, 'top', false, 'isHasFn', dataColumns, true);
				var sum = 0;
				for(var i = 0; i < dataColumns.length; i++) {
				...
				$("#cost_list_job_details").datagrid("loadData", joblist);
			} else {
				showErrorFunHandler(data[$.getRequestMetaName()].result, "ERROR", "ERROR");
			}
		},
...
function setJobdetailForeignMoneyCode(personMessage,item,foreigntype) {
		...
		//如果foreigntype有值,代表外货币种符号,需要循环
		 if(foreigntype!=undefined&&foreigntype!=null){//如果foreigntype时空字符串,代表没有填写外货,是本国货币,不代表发票国际化.
...
function OutOfPDF(pdfname){
	...
	if(pdfname=="deboursCreate"){
		OutPutPdfHandler(job_cd,'',input_no,'deboursCreate',"1") //立替更新
	}else if(pdfname=="deboursCreate"){
		OutPutPdfHandler(job_cd,'',input_no,'deboursCreate',"1") //立替登录
	}else if(pdfname=="confirmDebours"){
		OutPutPdfHandler(job_cd,'',input_no,'confirmDebours',"1") //立替承认
	}else if(pdfname=="onCostCreate"){
		OutPutPdfHandler(job_cd,'',input_no,'onCostCreate',"1") //振替更新
	}else if(pdfname=="onCostCreate"){
		OutPutPdfHandler(job_cd,'',input_no,'onCostCreate',"1") //振替登录
	}else if(pdfname=="confirmOncost"){
		OutPutPdfHandler(job_cd,'',input_no,'confirmOncost',"1") //振替承认
		...
		OutPutPdfHandler(job_cd,input_no,"","reportCreate","1","","jobDetails");;//发注书
	}
