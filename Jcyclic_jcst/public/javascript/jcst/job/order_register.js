...
$(function(){
	...
	layerClick('mes-edit','30%','655',2,'../common/jcst/invoice_edit.shtml');
					...
					return;
				 	...
				 	//更新条数
				 	 if(data[$.getRequestDataName()]['num']==1){
				 	 	url = "jcst/job_registration_list.html?view=init";//此处拼接内容
           ...
           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
			$.resetRequestID(data[$.getRequestUserInfoName()].requestID);
				if(data[$.getRequestDataName()]['validatePower']==='0'){
				 url = "jcst/job_registration_list.html?view=init";
				...
				$('#order_date').datebox('setValue','systemDate')//外发日	
				...
				//}
					$.layerShowDiv('icon-i','1000','600',2,'../common/jcst/cost_list.shtml',jobcd);
	           }else{
	           	showErrorFunHandler(data[$.getRequestMetaName()].result,"ERROR","ERROR");
	           }
       },
	...
	$.ajax({
		...
		success:function(data){
           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
				...
				SelectObj.setSelectID = ['invoice_type','invoice_text'];
				SelectObj.setStringFlg='';
