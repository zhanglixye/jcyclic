	...
	//判断是否是从倒还栏跳转到job一览中。
	 ...
	 if(view!="init"&&view!="cost"&&view!="top"&&view!="job"&&view!="jobdetailsearch"){
	 	...
	 	if(backPage=="checkOut"){
	 		...
	 		url = "jcst/top_registration.html";
	 	}
...
function searchJoblistInit (parList,selflg){
	...
	success:function(data){
       if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
			...
			SelectObj.setSelectID = ['dlvfalg','assign_flg','invpd','cost_finish_flg','rjpd'];
			...
			for(var i=0;i<total;i++){
			   ...
			   //如果壳上承认了，税率取，税率历史记录表中的数据
			    if(data[$.getRequestDataName()]['joblist'][i]['saleadmitdate']!=undefined&&data[$.getRequestDataName()]['joblist'][i]['saleadmitdate']!=null&&data[$.getRequestDataName()]['joblist'][i]['saleadmitdate']!=""){
			   ...
			   /* if(isCostFinsh=='0'){//未终了*/
					data[$.getRequestDataName()]['joblist'][i]['tax2']=conf['tax'];//税金2
					data[$.getRequestDataName()]['joblist'][i]['tax3']=conf['tax1']//税金3
...
function clickMenu(id){
	...
	if(id=='costlist'){//原价一览
		...
		url = $.getJumpPath()+"jcst/cost_list.html?jobcd="+jobcd;
		...
		url = $.getJumpPath()+"jcst/job_update.html?jobcd="+jobcd;
        window.location.href = url;
		...
		url = $.getJumpPath()+"jcst/salescategory_registration.html?jobcd="+jobcd;
        window.location.href = url;
	}
		...
		url = $.getJumpPath()+"jcst/salescategory_update.html?jobcd="+jobcd;
        window.location.href = url;
	}
		...
		url = $.getJumpPath()+"jcst/salescategory_approval.html?jobcd="+jobcd;
        window.location.href = url;
		...
		url = $.getJumpPath()+"jcst/order_register.html?jobcd="+jobcd;
        window.location.href = url;
	}else if(id=='liti'){//立体
		...
		url = $.getJumpPath()+"jcst/advance_registration.html?jobcd="+jobcd;
        window.location.href = url;
	}else if (id=='zhenti') {//振提
		url = $.getJumpPath()+"jcst/transfer_registration.html?jobcd="+jobcd;
        window.location.href = url;
		...
		url = $.getJumpPath()+"jcst/job_details.html?jobcd="+jobcd+"&jobdetailflag=joblist";
        window.location.href = url;
	}
}
//多条menu点击事件
		...
		success:function(data){
           if(data[$.getRequestMetaName()].result == $.getRequestStatusName()){
           	     ...
           	     if(data[$.getRequestDataName()]=="2"){
           	 		 showLockInfoMsgHandler("STATUS_VALIDATEPOWER_ERROR","jcst/job_registration_list.html?view=init&menu=se");
					 return;
				...
				//更新表格数据
				/*var view = getQueryString("view","jcst/job_registration_list.html");
	            var pars =JSON.parse(sessionStorage.getItem('pars'));
				var sideBar = urlPars.parm("sideBar");
				var parsList = jumpPageSearchList(view);
				if(view == "top" && parsList.keyword != "")
