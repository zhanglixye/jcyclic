...
//	if(saleAddFlag=='0'){
//	  $.layerShowDiv('job_detail','1000','650',2,'../jcst/job_detail_nosale.html');
...
//	            $('#itemList option:first').remove();
				...
				$('#userid').val(data.data.LendtrnInput.timesheetuser[0].userid)
				...
				//把本国货币小数点位数赋值到全局变量中
			 	...
			 	if(!data[$.getRequestDataName()].rate>0){
			 		showInfoMsgHandler('NOTAXRATE',"ERROR","ERROR","jcst/cost_list.html?view=6","");
			 		//goCostlist();
			 	}
			 	//判断原价情报是否可点击
...
//			 		showConfirmMsgHandler('NOTAXRATE');
					var urlad = 'jcst/job_registration_list.html?view=init&menu=se';
...
function setfroeginlength(){
	var languagefor = $('#cost_need_change_money1').text(); 
		for(var i=0;i<foreignLen.length;i++){
			if(languagefor==foreignLen[i].itmname){
