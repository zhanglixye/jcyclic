 ...
 * @date 日期 2018.05.16
 */
function changeClientLeftOrRight(flg, leftTabName, rightTabName, flag) {
	if(flg == "left") {
		//从左到右
		...
		if(rows.length > 0) {
			...
			for(var j = 0; j < rows.length; j++) {
				for(var i = 0; i < rowleft.length; i++) {
					var uscd = rows[j].account_cd;
					var arr_account_cd = rowleft[i].account_cd;
					if(uscd == undefined) {
						uscd = rows[j].user_cd;
		...
		if(rows.length > 0) {
			...
			for(var j = 0; j < rows.length; j++) {
				for(var i = 0; i < rowright.length; i++) {
					var uscd = rows[j].account_cd;
					var arr_account_cd = rowright[i].account_cd;
					if(uscd == undefined) {
						uscd = rows[j].user_cd;
	...
	$.ajax({
		...
		success: function(data) {
			if(data[$.getRequestMetaName()].result == $.getRequestStatusName()) {
				...
				initDataGridHandler(cost_list, null, null, 'top', false, '', dataColumns, true);
				var sum = 0;
				for(var i = 0; i < dataColumns.length; i++) {
 ...
 * @date 日期 2018.08.29
 */
function array_remove_repeat(a,b,c) { // 去重
	var r  = [];
	var e = [];
	for(var i = 0; i < a.length; i++) {
		var flag = true;
		...
		tempLevel= b[i];
			...
			if(temp == r[j].temp&&tempLevel==r[j].tempLevel) {
				flag = false;
				break;
			}
		}
		if(flag){
			lableRow = {"temp":temp,"tempLevel":tempLevel};
			r.push(lableRow);
...
function showLocale(obj) {
	...
	if($("#localtime").exist()) {
		$("#localtime").html(str);
		obj.a = new Date(obj.a.getTime() + 1000);
	}
}
...
$(function() {
	var href = window.location.href;
	if(href.match(/\/job_registration_list.html|\/job_registration.html|\/job_details.html|\/job_update.html|jcst\/salescategory_[a-z]+.html/) !== null) {
...
function part_language_change(dom) {
	...
	jQuery.i18n.properties({
		...
		path: '../public/language/jcst/' + i18nLanguage + '/', //资源文件路径
...
function init_login_language_load(i18nLanguage, flag) {
	...
	if(flag) {
		var path = '../public/language/jcst/' + i18nLanguage + '/';
	} else {
		var path = 'public/language/jcst/' + i18nLanguage + '/';
	}
	jQuery.i18n.properties({
...
function goBackcostlist(){
	url = "jcst/cost_list.html?view=6";//此处拼接内容
