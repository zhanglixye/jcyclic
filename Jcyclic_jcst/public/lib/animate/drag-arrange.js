...
(function (factory) {
  ...
  $.fn.arrangeable = function(options) {
	...
	var dragCheck  = options['dragCheck'];
    $elements = this;
    ...
    function dragMoveHandler(e) {
      ...
      if (dragging) {
        ...
        shiftHoveredElement($clone, $dragElement, $elements,move_check);
  ...
  function shiftHoveredElement($clone, $dragElement, $movableElements,move_check) {
    var hoveredElement = getHoveredElement($clone, $dragElement, $movableElements);
    //被触摸的对象的文字
	var hoveredText = $.trim($(hoveredElement).find('.list_of_items_bottom_top').text());
	var cloneText = $.trim($clone.find('.list_of_items_bottom_top').text());
	//add code myself --- 拖动筛选判断	
	if(typeof move_check == 'string'){
		if(cloneText == move_check){
			return;
		}else{
	...
	}else if(typeof move_check == 'object'){
		if(move_check.in_array(cloneText) && move_check.in_array(hoveredText)){			
			if (hoveredElement !== $dragElement[0]) {
		      // shift all other elements to make space for the dragged element
		      var hoveredElementIndex = $movableElements.index(hoveredElement);
		      var dragElementIndex = $movableElements.index($dragElement);
		...
		}else if(!move_check.in_array(cloneText) && !move_check.in_array(hoveredText)){
			if (hoveredElement !== $dragElement[0]) {
		      // shift all other elements to make space for the dragged element
		      var hoveredElementIndex = $movableElements.index(hoveredElement);
		      var dragElementIndex = $movableElements.index($dragElement);
