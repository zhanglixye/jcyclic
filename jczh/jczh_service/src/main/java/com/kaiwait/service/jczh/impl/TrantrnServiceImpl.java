...
public class TrantrnServiceImpl implements TrantrnService{
	...
	public TrantrnInput TrantrnQueryList(TrantrnInput inputParam)
	{
		...
		List<JobLableTrn> costlable = costMapper.selectCostLable(trantrnlist.get(0).getInput_no(),companyID,3);
	...
	public int TrantrnApprovalTx(TrantrnInput inputParam)
	{
		...
		jobLandMapper.deleteByJobCd(null, inputParam.getOld_input_no(), Integer.valueOf(inputParam.getCompanyID()),
				"orderlabeltrn");
		 if(jltrn==null||jltrn.size()==0) {
		    	jltrn = null;
		   }else {
		    	for(int i=0;i<jltrn.size();i++) {
					jltrn.get(i).setCostno(inputParam.getOld_input_no());
		 ...
		 accountEntriesService.createTranAccountsData(jobcd, inputParam.getOld_input_no(), companyID, inputParam.getUserID(), month);
...
//	        }
	...
	public int cancelTranTx(TrantrnInput trantrn) {
		...
		jobLandMapper.deleteByJobCd(null, trantrn.getOld_input_no(), Integer.valueOf(companyID),
				"orderlabeltrn");
		 if(jltrn==null||jltrn.size()==0) {
		    	jltrn = null;
		   }else {
		    	for(int i=0;i<jltrn.size();i++) {
					jltrn.get(i).setCostno(trantrn.getOld_input_no());
					jltrn.get(i).setLablelevel(3);
		 ...
		 TrantrnMapper.updateLock(jobcd,companyID,inputno);
		 //承认取消时，删除会计分录数据
		 accountEntriesService.dropAccountDatasByCancel(jobcd, trantrn.getOld_input_no(), companyID);
	...
	public String TrantrnApprovalPDF(OutPutInput jobLand) throws JRException {
		TrantrnApprovalBeen jobLandOut = TrantrnMapper.TrantrnApprovalPDF(jobLand.getJobNo(),Integer.valueOf(jobLand.getCompanyID()),jobLand.getInputNo(),jobLand.getLangTyp());
		
		jobLandOut.setApprovalDate(commonMethedService.getTimeByZone(jobLandOut.getApprovalDate(),jobLand.getCompanyID()).substring(0,10));
		String language = jobLand.getLangTyp();
		String currencycode = jobLandOut.getCurrencycode();
		String moneyType = jobLandOut.getMoneyType();
		currencycode=jobLandOut.getLocalMoney();
		...
		jobLandOut.setCurrencycode(currencycode);
		jobLandOut.setMoneyType(moneyType);
		if(jobLandOut.getTrantrnNum()!=null) {
			jobLandOut.setTrantrnNum(commonMethedService.getNewNum(jobLandOut.getTrantrnNum(),Integer.parseInt(jobLandOut.getMylen())));

		}
		if(jobLandOut.getDebtorAmt()!=null) {
			jobLandOut.setDebtorAmt(commonMethedService.getNewNum(jobLandOut.getDebtorAmt(),Integer.parseInt(jobLandOut.getMylen())));

		}
		if(jobLandOut.getCreditAmt()!=null) {
			jobLandOut.setCreditAmt(commonMethedService.getNewNum(jobLandOut.getCreditAmt(),Integer.parseInt(jobLandOut.getMylen())));
		}
