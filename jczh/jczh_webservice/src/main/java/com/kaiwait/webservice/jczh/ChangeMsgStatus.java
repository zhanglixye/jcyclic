package com.kaiwait.webservice.jczh;



import javax.annotation.Resource;

import org.springframework.stereotype.Component;

import com.kaiwait.bean.jczh.io.CostInput;
import com.kaiwait.core.process.SingleFunctionIF;
import com.kaiwait.core.process.ValidateResult;
import com.kaiwait.service.jczh.TopService;

@Component("changeMsgStatus")
public class ChangeMsgStatus implements SingleFunctionIF<CostInput>{
	
	@Resource
	private TopService topService;
	
	@Override
	public Object process(CostInput inputParam) {
		
		return topService.changeMsgStatusTx(inputParam.getMsgId(),Integer.valueOf(inputParam.getCompanyID()),inputParam.getUserID());
	}

	public ValidateResult validate(CostInput inputParam) {		
		return null;
	}

	@SuppressWarnings("rawtypes")
	@Override
	public Class getParamType() {
		// TODO Auto-generated method stub
		return CostInput.class;
	}


}
