...
//		company.setUpUsercd(logUsercd);
		...
		for(int i=0;i<commonVo.size();i++) {
				...
				if("0001".equals(commonVo.get(i).getMstcd())) {
					...
					String oldNow [] = now.split("-");
					now = oldNow[0]+"-"+oldNow[1]+"-01";
		...
		//查询是否有通货
		String CURRENCY_CODE= userMapper.selectcompanmst(companycd);
		//如果没有则去查询commonmst中无效的数据有多少条
		if(CURRENCY_CODE==null||CURRENCY_CODE.equals("")) {
			List<Commonmst> commst = userMapper.selectitemcd(companycd);
			if(commst.size()==1&&CURRENCY_CODE==null) {
				String itemcd = commst.get(0).getItemcd();
				companyMapper.upCompany(itemcd,companycd);
			}
		}
		companyMapper.insertListColumnMst(companycd);
		if(num>0) {
	...
	public Integer updateCompanyTx(CompanyMstInput inputParam){
		...
		if(!company.getChangeInvoiceFlg().equals("0")) {
			if(company.getJobInvoiceFlg() == 0||company.getJobInvoiceFlg() == 1)
			{
				String sysDate = companyMapper.getSystemDate(company.getCompany_cd());
				int isCreateInvoiceNoNums = companyMapper.getIsCompanyInvoiceFlg(company.getCompany_cd(),sysDate);
				if(isCreateInvoiceNoNums > 0)
				{
					return 540;
				}
			}
