...
public class SalemstSel implements SingleFunctionIF<SalemstInput>{
	...
	public List<SalemstVo> process(SalemstInput inputParam) {
		...
		return SalemstService.select(inputParam.getSale_cd(),inputParam.getSale_account_cd(),inputParam.getSale_name(),inputParam.getDel_flg(),inputParam.getCompanyID(),inputParam.getPd());
	}
